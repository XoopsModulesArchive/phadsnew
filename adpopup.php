...
function enjavanateCode($str, $limit = 60)
{
    ...
    while (mb_strlen($str) > 0) {
        ...
        while ('\\' == mb_substr($line, mb_strlen($line) - 1, 1)) {
            $line .= mb_substr($str, 0, 1);
...
function enjavanateBanner($output, $limit = 60)
{
    ...
    $ret .= "\tdocument.write('" . ($output['alt'] ?: 'Advertisement') . "');\n";
...
?>
    ...
    {
<?php echo $windowid; ?> =  window.open('', '<?php echo $windowid; ?>', 'height=<?php echo $output['height']; ?>,width=<?php echo $output['width']; ?>,toolbars=<?php echo 1 == $toolbars ? 'yes' : 'no'; ?>,location=<?php echo 1 == $location ? 'yes' : 'no'; ?>,menubar=<?php echo 1 == $menubar ? 'yes' : 'no'; ?>,status=<?php echo 1 == $status ? 'yes' : 'no'; ?>,resizable=<?php echo 1 == $resizable ? 'yes' : 'no'; ?>,scrollbars=<?php echo 1 == $scrollbars ? 'yes' : 'no'; ?>');
...
<?php
if (mb_strstr($HTTP_SERVER_VARS['HTTP_USER_AGENT'], 'MSIE') && !mb_strstr($HTTP_SERVER_VARS['HTTP_USER_AGENT'], 'Opera')) {
    ...
    echo enjavanateCode($output['alt'] ?: 'Advertisement');
...
<?php if (isset($popunder) && '1' == $popunder) { ?>
    window.focus();
<?php } else { ?>
