...
if ('english' != $phpAds_config['language'] && file_exists(phpAds_path . '/language/' . $phpAds_config['language'] . '/report.lang.php')) {
...
function phpAds_ReportPluginList($directory)
{
    ...
    while ($pluginFile = readdir($pluginDir)) {
        if (preg_match('^([a-zA-Z0-9\-]*)\.plugin\.php$', $pluginFile, $matches)) {
...
if ($fields = $plugin['plugin-import']) {
    ...
    for (reset($fields); $key = key($fields); next($fields)) {
        ...
        if ('edit' == $fields[$key]['type']) {
            ...
            echo $fields[$key]['size'] ?? '';

            echo "' value='";

            echo $fields[$key]['default'] ?? '';
        ...
        elseif ('delimiter' == $fields[$key]['type']) {
            echo "<tr><td width='30'>&nbsp;</td>";
        ...
        elseif ('campaignid-dropdown' == $fields[$key]['type']) {
            echo "<tr><td width='30'>&nbsp;</td>";
        ...
        elseif ('clientid-dropdown' == $fields[$key]['type']) {
            if (phpAds_isUser(phpAds_Client)) {
        ...
        elseif ('affiliateid-dropdown' == $fields[$key]['type']) {
            if (phpAds_isUser(phpAds_Affiliate)) {
                echo "<input type='hidden' name='" . $key . "' value='" . phpAds_getUserID() . "'>";
            } else {
                echo "<tr><td width='30'>&nbsp;</td>";
        ...
        elseif ('zoneid-dropdown' == $fields[$key]['type']) {
            echo "<tr><td width='30'>&nbsp;</td>";
