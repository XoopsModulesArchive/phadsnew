...
if (isset($submit)) {
    ...
    if (isset($unlimitedviews) && 'on' == mb_strtolower($unlimitedviews)) {
        $views = -1;
    }

    if (isset($unlimitedclicks) && 'on' == mb_strtolower($unlimitedclicks)) {
        $clicks = -1;
    }
    ...
    if ('t' == $active && !($targetviews > 0 || $weight > 0 || ('0000-00-00' != $expire && $views > 0))) {
        $active = 'f';
    }

    $new_campaign = 'null' == $campaignid;
    ...
    if ('t' == $active && '0000-00-00' != $expire && $views > 0) {
    ...
    if (!$new_campaign
        && ($active != $active_old
            || ('t' == $active && ($targetviews != $target_old || $weight != $weight_old)))) {
...
if ('' != $campaignid) {
    ...
    phpAds_ShowSections(['4.1.3.2', '4.1.3.3', '4.1.3.4']);
} else {
    if (isset($move) && 't' == $move) {
        ...
        phpAds_ShowSections(['4.1.3.1']);
    }
}

if ('' != $campaignid || (isset($move) && 't' == $move)) {
    // Edit or Convert
...
if ('t' == $row['active'] && '0000-00-00' != $row['expire'] && $row['views'] > 0) {
    $autotarget = true;
} else {
    $autotarget = false;
}
...
function phpAds_showDateEdit($name, $day = 0, $month = 0, $year = 0, $edit = true)
{
    ...
    if ($edit) {
        ...
        echo "<input type='radio' name='" . $name . "Set' value='f' onclick=\"phpAds_formDateClick('" . $name . "', false);\"" . (false === $set ? ' checked' : '') . " tabindex='" . ($tabindex++) . "'>";

        echo "&nbsp;$caption";

        echo '</td></tr><tr><td>';

        echo "<input type='radio' name='" . $name . "Set' value='t' onclick=\"phpAds_formDateClick('" . $name . "', true);\"" . (true === $set ? ' checked' : '') . " tabindex='" . ($tabindex++) . "'>";
...
echo "<input type='hidden' name='expire' value='" . ($row['expire'] ?? '') . "'>";
...
echo "<input type='hidden' name='active_old' value='" . (isset($row['active']) && 't' == $row['active'] ? 't' : 'f') . "'>";
...
if (isset($row['active']) && 'f' == $row['active']) {
    ...
    if (time() < mktime(0, 0, 0, $row['activate_month'], $row['activate_dayofmonth'], $row['activate_year'])) {
        $inactivebecause[] = $strBeforeActivate;
    }

    if (time() > $expire_ts && $expire_ts > 0) {
        $inactivebecause[] = $strAfterExpire;
    }

    if (0 == $row['target'] && 0 == $row['weight']) {
        $inactivebecause[] = $strWeightIsNull;
    }
    ...
    echo $strClientDeactivated . ' ' . implode(', ', $inactivebecause) . '.</div><br>';
...
echo "<input type='checkbox' name='unlimitedviews'" . (-1 == $row['views'] ? ' CHECKED' : '') . " onClick=\"phpAds_formUnlimitedClick('unlimitedviews', 'views');\" tabindex='" . ($tabindex++) . "'>&nbsp;";
echo $strUnlimited;
echo "</td></tr><tr><td><img src='images/spacer.gif' height='1' width='100%'></td>";
...
echo "<input type='checkbox' name='unlimitedclicks'" . (-1 == $row['clicks'] ? ' CHECKED' : '') . " onClick=\"phpAds_formUnlimitedClick('unlimitedclicks', 'clicks');\" tabindex='" . ($tabindex++) . "'>&nbsp;";
echo $strUnlimited;
echo "</td></tr><tr><td><img src='images/spacer.gif' height='1' width='100%'></td>";
echo "<td colspan='2'><img src='images/break-l.gif' height='1' width='200' vspace='6'></td></tr>";
...
echo "<tr><td valign='top'>";
echo "<input type='radio' name='priority' value='t'" . ('f' != $priority && !$autotarget ? ' checked' : '') . " onClick='phpAds_formPriorityClick(this.form, true)' tabindex='" . ($tabindex++) . "'>";
...
echo "<input onBlur='phpAds_formUpdate(this);' class='flat' type='text' name='targetviews' size='7' value='" . ($row['target'] ?? '-') . "' tabindex='" . ($tabindex++) . "'> ";
echo $strTargetPerDay . '<br><br></td></tr>';

echo "<tr><td valign='top'>";
echo "<input type='radio' name='priority' value='f'" . ('f' == $priority && !$autotarget ? ' checked' : '') . " onClick='phpAds_formPriorityClick(this.form, true)' tabindex='" . ($tabindex++) . "'>";
echo "</td><td valign='top'>" . $strLowPriority . "<br><img src='images/break-l.gif' height='1' width='100%' vspace='6'><br>";
echo $strCampaignWeight . ': ';
echo "<input onBlur='phpAds_formUpdate(this);' class='flat' type='text' name='weight' size='7' value='" . ($row['weight'] ?? $phpAds_config['default_campaign_weight']) . "' tabindex='" . ($tabindex++) . "'>";

echo "<input type='hidden' name='previousweight' value='" . ($row['weight'] ?? '') . "'>";
echo "<input type='hidden' name='previousactive' value='" . ($row['active'] ?? '') . "'>";
echo '</td></tr></table>';
echo '</td></tr>';
