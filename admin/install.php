...
if (mb_strlen(__FILE__) > mb_strlen(basename(__FILE__))) {
    define('phpAds_path', mb_substr(__FILE__, 0, (mb_strlen(__FILE__) - mb_strpos(strrev(__FILE__), strrev('admin')) - mb_strlen('admin') - 1)));
} else {
    define('phpAds_path', '..');
}
...
if (!count($_POST) && !count($_GET)) {
    ...
    if ($result = phpAds_revisionCheck()) {
        [$rev_direct, $rev_fatal, $rev_message] = $result;
...
// Set URL prefix
$host = $HTTP_SERVER_VARS['HTTP_HOST'] ?? $HTTP_SERVER_VARS['SERVER_NAME'];

$phpAds_config['url_prefix'] = mb_strtolower(
    eregi_replace(
                                       '^([a-z]+)/.*$',
                                       '\\1://',
                                       $HTTP_SERVER_VARS['SERVER_PROTOCOL']
                                   )
) . $host . preg_replace("/admin/install.php(\?.*)?$", '', $HTTP_SERVER_VARS['PHP_SELF']);
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    switch ($phase) {
        ...
        case 2:
            ...
            // Check file_uploads
            if (true !== ini_get('file_uploads')) {
                $fatal[] = $strWarningFileUploads;
            }

            // Check magic_quotes_sybase
            if (false !== ini_get('magic_quotes_sybase')) {
        ...
        case 3:
            ...
            if (!mb_strlen($admin) || !mb_strlen($admin_pw)) {
                $errormessage[0][] = $strInvalidUserPwd;
            }

            if (mb_strlen($admin_pw) && $admin_pw != $admin_pw2) {
            ...
            if (!(isset($arraybugcheck) && is_array($arraybugcheck) && 4 == mb_strlen($arraybugcheck[0]))) {
        ...
        case 4:
            ...
            // Check table prefix
            if (mb_strlen($table_prefix) && !eregi('^[a-z][a-z0-9_]*$', $table_prefix)) {
    ...
    if ($phase > 2) {
        echo "<b><a href='javascript:toggleHelp();'><img src='images/help-book.gif' width='15' height='15' border='0' align='absmiddle'>&nbsp;" . $strHelp . '</a></b>';
    }
    ...
    switch ($phase) {
        ...
        case 2:
            ...
            if (count($fatal) || isset($rev_direct)) {
                ...
                if (count($fatal)) {
                    for ($r = 0, $rMax = count($fatal); $r < $rMax; $r++) {
                        echo '<li>' . $fatal[$r] . '</li>';
                    }
                }

                if (isset($rev_direct)) {
                    for ($r = 0, $rMax = count($rev_message); $r < $rMax; $r++) {
...
'visible' => phpAds_tableTypesSupported,
    ...
    if (isset($fatal) && count($fatal)) {
        $btn = '';
    }                // Configuration errors -> allow a retry

    if (isset($rev_message) && 2 == $displayed_phase) {
        if ($rev_fatal) {
            $btn = '';
        }                                            // Fatal integrety error -> do not show
        elseif (!isset($fatal) || !count($fatal)) {                // Unless configuration error
            $btn = $GLOBALS['strIgnoreWarnings'];
        }            // Integrety warning -> ignore
    }
