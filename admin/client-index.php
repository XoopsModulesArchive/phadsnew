...
phpAds_ShowSections(['4.1', '4.2', '4.3']);
...
if (!isset($hideinactive)) {
    $hideinactive = $Session['prefs']['client-index.php']['hideinactive'] ?? ('t' == $phpAds_config['gui_hide_inactive']);
}
...
if (isset($clients) && is_array($clients) && count($clients) > 0) {
    ...
    while (list($key, $client) = each($clients)) {
        ...
        if (isset($client['campaigns']) && 0 == count($client['campaigns']) && $client['hideinactive'] > 0) {
            $clientshidden++;
...
if (!isset($clients) || !is_array($clients) || 0 == count($clients)) {
    ...
    for (reset($clients); $key = key($clients); next($clients)) {
        ...
        if (0 == $client['parent']) {
            ...
            if ((0 == $client['count'] && '1' == $client['expand']) || !isset($client['campaigns'])) {
            ...
            if (isset($client['campaigns']) && count($client['campaigns']) > 0 && '1' == $client['expand']) {
                ...
                for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
                    ...
                    echo "<a href='campaign-delete.php?clientid="
                         . $client['clientid']
                         . '&campaignid='
                         . $campaigns[$ckey]['clientid']
                    ...
                    if ('1' == $campaigns[$ckey]['expand'] && isset($campaigns[$ckey]['banners'])) {
                        ...
                        for (reset($banners); $bkey = key($banners); next($banners)) {
                            ...
                            echo "<a href='banner-acl.php?clientid="
                                 . $client['clientid']
                                 . '&campaignid='
                                 . $campaigns[$ckey]['clientid']
                                 . '&bannerid='
                                 . $banners[$bkey]['bannerid']
                            ...
                            echo "<a href='banner-delete.php?clientid="
                                 . $client['clientid']
                                 . '&campaignid='
                                 . $campaigns[$ckey]['clientid']
                                 . '&bannerid='
