...
if (!defined('phpAds_installing')) {
    ...
    if ('english' != $phpAds_config['language'] && file_exists(phpAds_path . '/language/' . $phpAds_config['language'] . '/settings.lang.php')) {
...
if ('english' != $phpAds_config['language'] && file_exists(phpAds_path . '/language/' . $phpAds_config['language'] . '/settings-help.lang.php')) {
    @include phpAds_path . '/language/' . $phpAds_config['language'] . '/settings-help.lang.php';
}

// Include required files
require __DIR__ . '/lib-config.inc.php';
...
function phpAds_SettingsSelection($section)
{
    ...
    echo "<option value='db'" . ('db' == $section ? ' selected' : '') . '>' . $GLOBALS['strDatabaseSettings'] . '</option>';

    echo "<option value='invocation'" . ('invocation' == $section ? ' selected' : '') . '>' . $GLOBALS['strInvocationAndDelivery'] . '</option>';

    echo "<option value='host'" . ('host' == $section ? ' selected' : '') . '>' . $GLOBALS['strHostAndGeo'] . '</option>';

    echo "<option value='stats'" . ('stats' == $section ? ' selected' : '') . '>' . $GLOBALS['strStatisticsSettings'] . '</option>';

    echo "<option value='banner'" . ('banner' == $section ? ' selected' : '') . '>' . $GLOBALS['strBannerSettings'] . '</option>';

    echo "<option value='admin'" . ('admin' == $section ? ' selected' : '') . '>' . $GLOBALS['strAdministratorSettings'] . '</option>';

    echo "<option value='interface'" . ('interface' == $section ? ' selected' : '') . '>' . $GLOBALS['strGuiSettings'] . '</option>';

    echo "<option value='defaults'" . ('defaults' == $section ? ' selected' : '') . '>' . $GLOBALS['strInterfaceDefaults'] . '</option>';
...
function phpAds_ShowSettings($data, $errors = [])
{
...
function phpAds_ShowSettings_CheckDependancies($data, $item)
{
    ...
    if (isset($item['depends'])) {
        $depends = preg_split('[ ]+', $item['depends']);
...
function phpAds_ShowSettings_StartSection($name, $error = [])
{
...
{
    if (!isset($item['size']) || '' == $item['size'] || 'small' == $item['size']) {
...
function phpAds_ShowSettings_Checkbox($item, $value)
{
    ...
    echo "<input type='checkbox' name='" . $item['name'] . "' value='true'" . (true === $value ? ' checked' : '') . ($item['enabled'] ? ' disabled' : '');
...
function phpAds_ShowSettings_Text($item, $value)
{
    ...
    echo "size='" . $item['size'] . "' value=\"" . htmlspecialchars($value, ENT_QUOTES | ENT_HTML5) . "\" onFocus=\"setHelp('" . $item['name'] . "')\" tabindex='" . ($tabindex++) . "'>";
...
function phpAds_ShowSettings_Textarea($item, $value)
{
    ...
    echo "style='width: 350px;' onFocus=\"setHelp('" . $item['name'] . "')\" tabindex='" . ($tabindex++) . "'>" . htmlspecialchars($value, ENT_QUOTES | ENT_HTML5) . '</textarea>';
...
function phpAds_ShowSettings_Select($item, $value)
{
    ...
    if (isset($item['items'])) {
        ...
        while (list($k, $v) = each($item['items'])) {
            echo '<option value="' . htmlspecialchars($k, ENT_QUOTES | ENT_HTML5) . '"' . ($k == $value ? ' selected' : '') . '>' . $v . '</option>';
        }
...
function phpAds_ShowSettings_Locked($item)
{
    ...
    if (!defined('phpAds_installing')
        && isset($item['name']) && isset($phpAds_settings_information[$item['name']]) && true !== $phpAds_settings_information[$item['name']]['sql']
        && true === $phpAds_config_locked) {
        return true;
    }
