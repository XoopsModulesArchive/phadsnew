...
if (isset($action)) {
    ...
    if (isset($action['new'])) {
        ...
        if ('time' == $type || 'weekday' == $type || 'browser' == $type || 'os' == $type
            || 'country' == $type || 'continent' == $type || 'region' == $type
            || 'language' == $type) {
            $acl[$last]['data'] = [];
        } elseif ('date' == $type) {
            $acl[$last]['data'] = ['day' => '-', 'month' => '-', 'year' => '-'];
        } else {
            $acl[$last]['data'] = '';
        }
    }
        ...
        if (isset($acl) && count($acl)) {
            ...
            while (list($key, ) = each($acl)) {
                if (isset($acl[$key]['data'])) {
                    if ('time' == $acl[$key]['type'] || 'weekday' == $acl[$key]['type']
                        || 'country' == $acl[$key]['type'] || 'continent' == $acl[$key]['type']
                        || 'region' == $acl[$key]['type']) {
                        $acl[$key]['data'] = implode(',', $acl[$key]['data']);
                    } elseif ('language' == $acl[$key]['type']) {
                        $acl[$key]['data'] = '(' . implode(')|(', $acl[$key]['data']) . ')';
                    } elseif ('browser' == $acl[$key]['type']) {
                        ...
                        $acl[$key]['data'] = addslashes('(' . implode(')|(', $regs) . ')');
                    } elseif ('date' == $acl[$key]['type']) {
                        $acl[$key]['data'] = sprintf(
                            '%04d%02d%02d',
                            $acl[$key]['data']['year'],
                            $acl[$key]['data']['month'],
                            $acl[$key]['data']['day']
...
if (!isset($acl) && $phpAds_config['acl']) {
    ...
    while (false !== ($row = phpAds_dbFetchArray($res))) {
        ...
        if ('time' == $row['type'] || 'weekday' == $row['type']
            || 'country' == $row['type'] || 'continent' == $row['type']
            || 'region' == $row['type']) {
            $acl[$row['executionorder']]['data'] = explode(',', $row['data']);
        } // Languages

        elseif ('language' == $row['type']) {
            if (preg_match("^\(.*\)$", $row['data'])) {
                $row['data'] = mb_substr($row['data'], 1, -1);
            }

            $acl[$row['executionorder']]['data'] = explode(')|(', $row['data']);
        } // Browsers

        elseif ('browser' == $row['type']) {
            if (preg_match("^\(.*\)$", $row['data'])) {
                $row['data'] = mb_substr($row['data'], 1, -1);
            }
            ...
            while (list($k, $v) = each($keys)) {
                ...
                while (list($tk, $tv) = each($phpAds_Browser)) {
                    if ($tv == $v) {
                        $acl[$row['executionorder']]['data'][] = $tk;
                    }
                }
            }
        } // Operating systems
        ...
        elseif ('os' == $row['type']) {
            ...
            if ('00000000' == $row['data']) {
                $acl[$row['executionorder']]['data'] = [
                    'day' => '-',
'month' => '-',
'year' => '-',
                ];
                ...
                $acl[$row['executionorder']]['data'] = [
                    'day' => mb_substr($row['data'], 6, 2),
'month' => mb_substr($row['data'], 4, 2),
'year' => mb_substr($row['data'], 0, 4),
                ];
            }
        } // Others
...
if ($phpAds_config['acl']) {
    ...
    if (isset($acl) && count($acl)) {
        ...
        while (list($key, ) = each($acl)) {
            if ('or' == $acl[$key]['logical'] && $current_i > 0) {
                echo "<tr><td colspan='4'><img src='images/break-el.gif' width='100%' height='1'></td></tr>";
            ...
            $bgcolor = 0 == $previous_i % 2 ? '#F6F6F6' : '#FFFFFF';
            ...
            if ('date' == $acl[$key]['type']) {
                $comparison_list = $comparison_date;
            } elseif ('referer' == $acl[$key]['type']) {
            ...
            if ('weekday' == $acl[$key]['type']) {
                ...
                for ($i = 0; $i < 7; $i++) {
                    ...
                    echo "<td><input type='checkbox' name='acl[" . $key . "][data][]' value='$i'" . (in_array($i, $acl[$key]['data'], true) ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;" . $strDayShortCuts[$i] . '&nbsp;&nbsp;</td>';
                ...
                for ($i = 0; $i < 24; $i++) {
                    ...
                    echo "<td><input type='checkbox' name='acl[" . $key . "][data][]' value='$i'" . (in_array($i, $acl[$key]['data'], true) ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;" . $i . ':00&nbsp;&nbsp;</td>';
                ...
                while (list($iso, $fullname) = each($phpAds_ISO639)) {
                    ...
                    echo "type='checkbox' id='check_" . $key . '_' . $iso . "' name='acl[" . $key . "][data][]' value='$iso'" . (in_array($iso, $acl[$key]['data'], true) ? ' checked' : '') . " align='middle'>" . $fullname;

                    echo '</div>';
                }

                echo '</div>';
            } elseif ('country' == $acl[$key]['type']) {
                if (!isset($acl[$key]['data'])) {
                    $acl[$key]['data'] = [];
                }
                ...
                while (list($iso, $fullname) = each($phpAds_ISO3166)) {
                    ...
                    echo "type='checkbox' id='c_" . $key . '_' . $iso . "' name='acl[" . $key . "][data][]' value='$iso'" . (in_array($iso, $acl[$key]['data'], true) ? ' checked' : '') . '>' . $fullname;

                    echo '</div>';
                }

                echo '</div>';
            } elseif ('continent' == $acl[$key]['type']) {
                if (!isset($acl[$key]['data'])) {
                    $acl[$key]['data'] = [];
                }
                ...
                while (list($postalcode, $fullname) = each($phpAds_USStates)) {
                    ...
                    echo "type='checkbox' id='check_" . $key . '_' . $postalcode . "' name='acl[" . $key . "][data][]' value='$postalcode'" . (in_array($postalcode, $acl[$key]['data'], true) ? ' checked' : '') . " align='middle'>" . $fullname;
                ...
                while (list($ukey, $uvalue) = each($phpAds_Browser)) {
                    ...
                    echo "<td><input type='checkbox' name='acl[" . $key . "][data][]' value='" . $ukey . "'" . (in_array($ukey, $acl[$key]['data'], true) ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;" . $ukey . '&nbsp;&nbsp;</td>';
                ...
                echo "<input type='text' size='40' name='acl[" . $key . "][data]' value='" . phpAds_htmlQuotes(stripslashes($acl[$key]['data'] ?? '')) . "' tabindex='" . ($tabindex++) . "'>";
            }
    ...
    while (list($type_id, $type_name) = each($type_list)) {
        ...
        printf("'%s'%s>", $type_id, 'weekday' == $type_id ? ' selected' : '');
