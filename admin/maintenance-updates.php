...
$current .= $strRunningOn . ' ' . str_replace('/', '&nbsp;', preg_replace(' .*$', '', $HTTP_SERVER_VARS['SERVER_SOFTWARE'])) . ', ';
...
if (!isset($Session['maint_update'])) {
    ...
    if ($maint_update[0] > 0 && 800 != $maint_update[0]) {
        phpAds_Die($strErrorOccurred, $strUpdateServerDown);
    }

    echo '<br><br>';

    if (800 == $maint_update[0]) {
        ...
        if (1 == $maint_update[1]['security_fix']) {
