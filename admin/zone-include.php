...
if (isset($zoneid) && '' != $zoneid) {
    if (isset($action) && 'toggle' == $action) {
        ...
        if (phpAds_ZoneBanners == $zonetype) {
            if (isset($bannerid) && '' != $bannerid) {
                phpAds_ToggleBannerInZone($bannerid, $zoneid);
            }
        }

        if (phpAds_ZoneCampaign == $zonetype) {
    ...
    if (isset($action) && 'set' == $action) {
        ...
        if (phpAds_ZoneBanners == $zonetype) {
            if (isset($bannerid) && is_array($bannerid)) {
                for ($i = 0, $iMax = count($bannerid); $i < $iMax; $i++) {
        ...
        if (phpAds_ZoneCampaign == $zonetype) {
            if (isset($clientid) && is_array($clientid)) {
                for ($i = 0, $iMax = count($clientid); $i < $iMax; $i++) {
...
if (!isset($hideinactive)) {
    $hideinactive = $Session['prefs']['zone-include.php']['hideinactive'] ?? ('t' == $phpAds_config['gui_hide_inactive']);
}

if (!isset($showbanners)) {
    $showbanners = $Session['prefs']['zone-include.php']['showbanners'] ?? ('t' == $phpAds_config['gui_show_matching']);
}

if (!isset($showcampaigns)) {
    $showcampaigns = $Session['prefs']['zone-include.php']['showcampaigns'] ?? ('t' == $phpAds_config['gui_show_parents']);
}
...
function phpAds_showZoneCampaign($width, $height, $what, $delivery)
{
    ...
    for ($k = 0, $kMax = count($what_array); $k < $kMax; $k++) {
        if ('clientid:' == mb_substr($what_array[$k], 0, 9)) {
            $clientid = mb_substr($what_array[$k], 9);
    ...
    if (!$compact) {
        ...
        if (isset($GLOBALS['clientid']) && '' != $GLOBALS['clientid']) {
            ...
            while (false !== ($row = phpAds_dbFetchArray($res))) {
                if (!isset($clientids[$row['clientid']]) || true !== $clientids[$row['clientid']]) {
    ...
    if (!$compact && 0 == phpAds_dbNumRows($res)) {
        ...
        for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
            ...
            $linkedrow = (isset($clientids[$campaign['clientid']]) && true === $clientids[$campaign['clientid']]);

            if ($compact) {
                $showrow = $linkedrow;
            } else {
                $showrow = (false === $hideinactive || true === $hideinactive && ('t' == $campaign['active'] || $linkedrow));
            }
            ...
            if ($showrow) {
                ...
                if (!$compact) {
                    ...
                    if (isset($clientids[$campaign['clientid']]) && true === $clientids[$campaign['clientid']]) {
                ...
                if ($showbanners && isset($campaign['banners'])) {
                    ...
                    while (list($bannerid, $banner) = each($campaign['banners'])) {
                        ...
                        if ('t' == $campaign['active'] && 't' == $banner['active']) {
                            if ('html' == $banner['storagetype']) {
                                echo "<img src='images/icon-banner-html.gif' align='absmiddle'>&nbsp;";
                            } elseif ('url' == $banner['storagetype']) {
                                echo "<img src='images/icon-banner-url.gif' align='absmiddle'>&nbsp;";
                            } elseif ('txt' == $banner['storagetype']) {
    ...
    if (!$compact) {
        ...
        echo '<tr ' . (0 == $i % 2 ? "bgcolor='#F6F6F6'" : '') . "><td height='25'>";

        echo "&nbsp;&nbsp;<input type='checkbox' name='checkall' value=''" . (true === $checkedall ? ' checked' : '') . " onclick='toggleall();' tabindex='" . ($tabindex++) . "'>";
...
function phpAds_showZoneBanners($width, $height, $what, $zonetype, $delivery)
{
    ...
    if (phpAds_ZoneBanners == $zonetype) {
        ...
        for ($k = 0, $kMax = count($what_array); $k < $kMax; $k++) {
            if ('bannerid:' == mb_substr($what_array[$k], 0, 9)) {
                $bannerid = mb_substr($what_array[$k], 9);

                $bannerids[$bannerid] = true;
            }
        }
    } elseif (phpAds_ZoneCampaign == $zonetype) {
    ...
    if (!$compact) {
        ...
        if (isset($GLOBALS['clientid']) && '' != $GLOBALS['clientid']) {
            ...
            if (isset($GLOBALS['campaignid']) && '' != $GLOBALS['campaignid']) {
                ...
                while (false !== ($row = phpAds_dbFetchArray($res))) {
                    if (!isset($bannerids[$row['bannerid']]) || true !== $bannerids[$row['bannerid']]) {
                        $name = $strUntitled;
        ...
        for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
            ...
            if (isset($campaign['banners']) && is_array($campaign['banners']) && count($campaign['banners'])) {
                ...
                for (reset($banners); $bkey = key($banners); next($banners)) {
                    ...
                    $linkedrow = (isset($bannerids[$banner['bannerid']]) && true === $bannerids[$banner['bannerid']]);

                    if ($compact) {
                        $showrow = $linkedrow;
                    } else {
                        $showrow = (false === $hideinactive || true === $hideinactive && ('t' == $banner['active'] && 't' == $campaign['active'] || $linkedrow));
                    }
                ...
                for (reset($banners); $bkey = key($banners); next($banners)) {
                    ...
                    if ($showrow) {
                        ...
                        if (!$compact) {
                            if (isset($bannerids[$banner['bannerid']]) && true === $bannerids[$banner['bannerid']]) {
...
echo "<option value='" . phpAds_ZoneCampaign . "'" . ((phpAds_ZoneCampaign == $zonetype) ? ' selected' : '') . '>' . $strCampaignSelection . '</option>';
echo "<option value='" . phpAds_ZoneBanners . "'" . ((phpAds_ZoneBanners == $zonetype) ? ' selected' : '') . '>' . $strBannerSelection . '</option>';
echo "<option value='" . phpAds_ZoneRaw . "'" . ((phpAds_ZoneRaw == $zonetype) ? ' selected' : '') . '>' . $strRawQueryString . '</option>';
echo '</select>';
...
if (phpAds_ZoneRaw == $zonetype) {
    ...
    echo "<textarea cols='50' rows='16' name='what' style='width:600px;' tabindex='" . ($tabindex++) . "'>" . ($zone['what'] ?? '') . '</textarea>';
