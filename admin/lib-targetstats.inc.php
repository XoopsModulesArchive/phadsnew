...
require __DIR__ . '/lib-gd.inc.php';
...
if (isset($row['span']) && $row['span'] > 0) {
    ...
    if ('w' == $period) {
        ...
        $shift = date('w') - ($phpAds_config['begin_of_week'] ? 1 - (0 == date('w') ? 7 : 0) : 0);
    ...
    while (false !== ($row = phpAds_dbFetchArray($result))) {
        $stats[$row['date']]['sum_views'] = $row['sum_views'];

        $stats[$row['date']]['sum_target'] = $row['sum_target'];

        $stats[$row['date']]['modified'] = 0 != $row['modified'];
    }
    ...
    if (isset($lib_targetstats_params)) {
        for (reset($lib_targetstats_params); $key = key($lib_targetstats_params); next($lib_targetstats_params)) {
            if (!(isset($lib_targetstats_misc_stats) && $lib_targetstats_misc_stats) && 'type' != $key) {
    ...
    echo "<option value='d'" . ('d' == $period ? ' selected' : '') . '>' . $strDailyHistory . '</option>';

    echo "<option value='w'" . ('w' == $period ? ' selected' : '') . '>' . $strWeeklyHistory . '</option>';

    echo "<option value='m'" . ('m' == $period ? ' selected' : '') . '>' . $strMonthlyHistory . '</option>';
    ...
    if ('m' == $period || 'd' == $period) {
        ...
        for ($d = 0; $d < $limit; $d++) {
            switch ($period) {
                case 'm':
                    $timestamp = mktime(0, 0, 0, date('m') - $d - $start, 1, date('Y'));
                    $span = mktime(0, 0, 0, date('m', $span), 1, date('Y', $span));
                    break;
                case 'd':
                    $timestamp = mktime(0, 0, 0, date('m'), date('d') - $d - $start, date('Y'));
            ...
            if (isset($stats[$key])) {
                $modified = $stats[$key]['modified'] ?? false;

                $target = $stats[$key]['sum_target'] ?? 0;

                $views = $stats[$key]['sum_views'] ?? 0;
            ...
            if (isset($lib_targetstats_hourlyurl) && 'd' == $period && $available) {
            ...
            if ($modified) {
                echo "<img src='images/warning.gif' align='absmiddle' alt='" . ('d' == $period ? $strTargetModifiedDay : $strTargetModifiedMonth) . "'>&nbsp;";
            }
        ...
        for ($i = 0, $iMax = count($limits); $i < $iMax; $i++) {
            if ($limit == $limits[$i]) {
        ...
        echo "<td align='"
             ...
             . "' height='25'>("
             . number_format(($totals['target'] ? $totaltarget / $totals['target'] * 100 : 0), $phpAds_config['percentage_decimals'], $phpAds_DecimalPoint, $phpAds_ThousandsSeperator)
             . '%)&nbsp;&nbsp;'
             . phpAds_formatNumber($totaltarget)
             . '</td>';

        echo "<td align='"
             . $phpAds_TextAlignRight
             . "' height='25'>("
             . number_format(($totals['views'] ? $totalviews / $totals['views'] * 100 : 0), $phpAds_config['percentage_decimals'], $phpAds_DecimalPoint, $phpAds_ThousandsSeperator)
             . '%)&nbsp;&nbsp;'
             . phpAds_formatNumber($totalviews)
             . '</td>';
    ...
    if ('w' == $period) {
        ...
        for ($d = 0; $d < $limit; $d++) {
            ...
            $week_formatted = '' != strftime('%V') ? strftime($weekiso_format, $week_timestamp + ($phpAds_config['begin_of_week'] ? 0 : (60 * 60 * 24))) : strftime($week_format, $week_timestamp + ($phpAds_config['begin_of_week'] ? 0 : (60 * 60 * 24)));
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                if (isset($stats[$key])) {
                    $modified[$i] = $stats[$key]['modified'] ?? false;

                    if ($modified[$i]) {
                        $modifiedweek = true;
                    }

                    $views[$i] = $stats[$key]['sum_views'] ?? 0;

                    $target[$i] = $stats[$key]['sum_target'] ?? 0;
