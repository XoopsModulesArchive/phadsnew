...
function phpAds_checkForUpdates($already_seen = 0)
{
    ...
    $client = new xmlrpc_client(
        $phpAds_updatesServer['script'],
        $phpAds_updatesServer['host'],
        $phpAds_updatesServer['port']
    );
    ...
    if ($response = $client->send($msg, 10)) {
        ...
        if (!$response->faultCode()) {
            $ret = [0, phpAds_xmlrpcDecode($response->value())];
            ...
            $ret = [$response->faultCode(), $response->faultString()];
        }

        return $ret;
    }

    return [-1, 'No response from the server'];
}
