...
if (isset($submit)) {
    ...
    if (phpAds_isUser(phpAds_Admin)) {
        ...
        if (isset($username) && '' != $username) {
            $res = phpAds_dbQuery(
				...
				WHERE
					LOWER(clientusername) = '" . mb_strtolower($username) . "'
			"
            ) or phpAds_sqlDie();

            $duplicateclient = (phpAds_dbNumRows($res) > 0);

            $duplicateadmin = (mb_strtolower($phpAds_config['admin']) == mb_strtolower($username));
            ...
            if ('' == $affiliateid) {
                $res = phpAds_dbQuery(
					...
					FROM
						' . $phpAds_config['tbl_affiliates'] . "
					WHERE
						LOWER(username) = '" . mb_strtolower($username) . "'
				"
                ) or phpAds_sqlDie();
                ...
                $res = phpAds_dbQuery(
					...
					FROM
						' . $phpAds_config['tbl_affiliates'] . "
					WHERE
						LOWER(username) = '" . mb_strtolower($username) . "' AND
						affiliateid != '$affiliateid'
					"
                ) or phpAds_sqlDie();
        ...
        if (isset($affiliatepermissions) && is_array($affiliatepermissions)) {
            for ($i = 0, $iMax = count($affiliatepermissions); $i < $iMax; $i++) {
    ...
    if (0 == count($errormessage)) {
        if ($affiliateid && (!isset($publiczones) || 't' != $publiczones) && 't' == $publiczones_old) {
            ...
            while (false !== ($currentrow = phpAds_dbFetchArray($res))) {
            ...
            if (count($zones)) {
                ...
                while (false !== ($currentrow = phpAds_dbFetchArray($res))) {
                    ...
                    if (in_array($append[0]['zoneid'], $zones, true)) {
                        phpAds_dbQuery(
                            '
								UPDATE
									' . $phpAds_config['tbl_zones'] . '
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    echo "<td width='100%'>" . ($affiliate['name'] ?? '');
}
...
echo "<input type='checkbox' name='publiczones' value='t'" . ('t' == $affiliate['publiczones'] ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;";
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    echo "<td width='370'>" . ($affiliate['username'] ?? '');
}
