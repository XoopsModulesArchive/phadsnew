...
if (!isset($hideinactive)) {
    $hideinactive = $Session['prefs']['client-campaigns.php'][$clientid]['hideinactive'] ?? ('t' == $phpAds_config['gui_hide_inactive']);
}
...
if (isset($campaigns) && is_array($campaigns) && count($campaigns) > 0) {
    ...
    while (list($key, $campaign) = each($campaigns)) {
        ...
        if (true === $hideinactive
            && ('f' == $campaign['active']
                || 't' == $campaign['active'] && 0 == count($campaign['banners'])
                   && count($campaign['banners']) < $campaign['count'])) {
            $campaignshidden++;
...
if (!isset($campaigns) || !is_array($campaigns) || 0 == count($campaigns)) {
    ...
    for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
        ...
        if ($phpAds_config['gui_show_campaign_info']) {
            ...
            echo '<tr ' . (0 == $i % 2 ? "bgcolor='#F6F6F6'" : '') . "><td colspan='1'>&nbsp;</td><td colspan='4'>";
            ...
            echo "<tr height='15'><td width='50%'>" . $strActivationDate . ': ' . ('0000-00-00' != $campaigns[$ckey]['activate'] ? $campaigns[$ckey]['activate_f'] : '-') . '</td>';

            echo "<td width='50%'>" . $strExpirationDate . ': ' . ('0000-00-00' != $campaigns[$ckey]['expire'] ? $campaigns[$ckey]['expire_f'] : '-') . '</td></tr>';
