...
function phpAds_getBannerCache($banner)
{
    ...
    if ('network' == $banner['storagetype']) {
        $buffer = phpAds_parseNetworkInfo($buffer);
    }

    // Auto change HTML banner

    if ('html' == $banner['storagetype'] || 'network' == $banner['storagetype']) {
        if ('t' == $banner['autohtml'] && $phpAds_config['type_html_auto']) {
            if ('' != $buffer) {
                ...
                if ($formpresent) {
                    ...
                    $buffer = eregi_replace(
                        "(<form([^>]*)action=['|\"]{0,1})([^'|\"|[:space:]]+)(['|\"]{0,1}([^>]*)>)",
                        '\\1{url_prefix}/adclick.php\\4' . "<input type='hidden' name='dest' value='\\3'>" . "<input type='hidden' name='bannerid' value='{bannerid}'>" . "<input type='hidden' name='source' value='{source}'>" . "<input type='hidden' name='zoneid' value='{zoneid}'>",
                        $buffer
                    );
                ...
                if ($linkpresent || $areapresent) {
                    ...
                    $lowerbanner = mb_strtolower($buffer);

                    $hrefpos = mb_strpos($lowerbanner, 'href=');
                    ...
                    while ($hrefpos > 0) {
                        ...
                        while ('<' != mb_substr($lowerbanner, $tagpos - 1, 1)) {
                            $tagpos--;
                        }

                        // travel up to next space

                        while (' ' != mb_substr($lowerbanner, $tagpos + $taglength, 1)) {
                            $taglength++;
                        }

                        $tag = mb_substr($lowerbanner, $tagpos, $taglength);
                        ...
                        if ('link' != $tag && 'base' != $tag) {
                            $hrefpos += 5;

                            $doublequotepos = mb_strpos($lowerbanner, '"', $hrefpos);

                            $singlequotepos = mb_strpos($lowerbanner, "'", $hrefpos);
                            ...
                            if ($quotepos > 0) {
                                $endquotepos = mb_strpos($lowerbanner, $quotechar, $quotepos + 1);

                                if ('{targeturl' != mb_substr($buffer, $quotepos + 1, 10) && 'javascript:' != mb_strtolower(mb_substr($buffer, $quotepos + 1, 11)) && 'mailto:' != mb_strtolower(mb_substr($buffer, $quotepos + 1, 7))) {
                                    $newbanner .= mb_substr($buffer, $prevhrefpos, $hrefpos - $prevhrefpos) . $quotechar . '{url_prefix}/adclick.php?bannerid=' . '{bannerid}&amp;zoneid={zoneid}&amp;source={source}&amp;dest=' . urlencode(
                                            mb_substr($buffer, $quotepos + 1, $endquotepos - $quotepos - 1)
                                        ) . '&amp;ismap=';
                                } else {
                                    $newbanner .= mb_substr($buffer, $prevhrefpos, $hrefpos - $prevhrefpos) . $quotechar . mb_substr($buffer, $quotepos + 1, $endquotepos - $quotepos - 1);
                                }

                                $prevhrefpos = $hrefpos + ($endquotepos - $quotepos);
                            } else {
                                $spacepos = mb_strpos($lowerbanner, ' ', $hrefpos + 1);

                                $endtagpos = mb_strpos($lowerbanner, '>', $hrefpos + 1);
                                ...
                                if ('{targeturl' != mb_substr($buffer, $hrefpos, 10) && 'javascript:' != mb_strtolower(mb_substr($buffer, $hrefpos, 11)) && 'mailto:' != mb_strtolower(mb_substr($buffer, $hrefpos, 7))) {
                                    $newbanner .= mb_substr($buffer, $prevhrefpos, $hrefpos - $prevhrefpos) . '"' . '{url_prefix}/adclick.php?bannerid=' . '{bannerid}&amp;zoneid={zoneid}&amp;source={source}&amp;dest=' . urlencode(mb_substr($buffer, $hrefpos, $endpos - $hrefpos)) . '&amp;ismap="';
                                } else {
                                    $newbanner .= mb_substr($buffer, $prevhrefpos, $hrefpos - $prevhrefpos) . '"' . mb_substr($buffer, $hrefpos, $endpos - $hrefpos) . '"';
                                }
                        ...
                        $hrefpos = mb_strpos($lowerbanner, 'href=', $hrefpos + 1);
                    }

                    $buffer = $newbanner . mb_substr($buffer, $prevhrefpos);
        ...
        if (mb_strpos($buffer, '{targeturl:') > 0) {
            while (eregi("\{targeturl:([^\}]*)\}", $buffer, $regs)) {
                if (false !== mb_strpos($regs[1], '|source:')) {
                    [$url, $source] = explode('|source:', $regs[1]);

                    if ('+' == mb_substr($source, 0, 1)) {
                        $source = '{source}-' . mb_substr($source, 1, mb_strlen($source) - 1);
                    }
                } else {
                    $source = '{source}';
    ...
    if ('sql' == $banner['storagetype'] || 'web' == $banner['storagetype'] || 'url' == $banner['storagetype']) {
        $buffer = str_replace('{imageurl}', $banner['imageurl'], $buffer);
    }

    // Set flash variables

    if ('swf' == $banner['contenttype']) {
        if ('' != $banner['url']) {
        ...
        while (eregi("\{targeturl:([^\}]*)\}", $buffer, $matches)) {
            if (false !== mb_strpos($matches[1], '|source:')) {
                [$url, $source] = explode('|source:', $matches[1]);
    ...
    if (eregi("\{pluginversion:([^\}]*)\}", $buffer, $matches)) {
        ...
        if ('swf' == $banner['contenttype'] && $banner['pluginversion'] > 4) {
        ...
        while ($networkfile = @readdir($networkdir)) {
            if (!@is_dir(phpAds_path . '/admin/networks/' . $networkfile)) {
                if (eregi("^([a-z0-9 -]+)\.html$", $networkfile, $fmatches)) {
                    ...
                    if (preg_match("\[title\]([^\[]*)\[\/title\]", $networkinfo, $tmatches)) {
...
function phpAds_setNetworkInfo($networkinfo, $vars)
{
    for (reset($vars); $key = key($vars); next($vars)) {
        if (preg_match("\[value:" . $key . "\]([^\[]*)\[\/value\]", $networkinfo, $matches)) {
...
{
    while (preg_match("\[value:([a-zA-Z0-9]*)\]([^\[]*)\[\/value\]", $networkinfo, $matches)) {
        $networkinfo = str_replace('{var:' . $matches[1] . '}', $matches[2], $networkinfo);

        $networkinfo = str_replace($matches[0], '', $networkinfo);
    }

    $networkinfo = preg_replace("\[properties\](.*)\[\/properties\]", '', $networkinfo);
...
function phpAds_getNetworkInfo($networkinfo)
{
    ...
    if (preg_match("\[title\]([^\[]*)\[\/title\]", $networkinfo, $matches)) {
        $result['title'] = $matches[1];
    }

    if (preg_match("\[logo\](.*)\[\/logo\]", $networkinfo, $matches)) {
        $result['logo'] = $matches[1];
    }

    if (preg_match("\[signup\](.*)\[\/signup\]", $networkinfo, $matches)) {
        $result['signup'] = $matches[1];
    }

    if (preg_match("\[login\](.*)\[\/login\]", $networkinfo, $matches)) {
        $result['login'] = $matches[1];
    }

    if (preg_match("\[comments\](.*)\[\/comments\]", $networkinfo, $matches)) {
        $result['comments'] = $matches[1] . " <a href='" . $result['signup'] . "' target='_blank'>" . $strMoreInformation . '</a><br><br>';
    }

    if (preg_match("\[ability:dynamic\]true\[\/ability\]", $networkinfo)) {
    ...
    if (preg_match("\[ability:track-clicks\]true\[\/ability\]", $networkinfo)) {
    ...
    if (preg_match("\[width\](.*)\[\/width\]", $networkinfo, $matches)) {
        $result['width'] = $matches[1];
    }

    if (preg_match("\[height\](.*)\[\/height\]", $networkinfo, $matches)) {
    ...
    while (preg_match("\[title:([a-zA-Z0-9]*)\]([^\[]*)\[\/title\]", $networkinfo, $tmatches)) {
        if (preg_match("\[value:" . $tmatches[1] . "\]([^\[]*)\[\/value\]", $networkinfo, $vmatches)) {
            $value = $vmatches[1];
        } else {
            $value = '';
        }

        $vars[] = ['name' => $tmatches[1], 'title' => $tmatches[2], 'default' => $value];
