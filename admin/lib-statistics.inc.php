...
{
    return mb_strlen($str) > $maxLen ? rtrim(mb_substr($str, 0, $maxLen - mb_strlen($append))) . $append : $str;
}
...
function phpAds_replaceVariablesInBannerCode($htmlcode)
{
    ...
    while (preg_match('#(%7B|\{)random((%3A|:)([0-9]+)){0,1}(%7D|})#i', $htmlcode, $matches)) {
        ...
        if (isset($lastdigits) && $lastdigits == $randomdigits) {
            ...
            for ($r = 0; $r < $randomdigits; $r += 9) {
                $randomnumber .= (string)random_int(111111111, 999999999);
            }

            $randomnumber = mb_substr($randomnumber, 0 - $randomdigits);
        }
    ...
    if ($phpAds_config['type_html_php']) {
        if (preg_match("#(\<\?php(.*)\?\>)#si", $htmlcode, $parser_regs)) {
            ...
            for ($parser_i = 0, $parser_iMax = count($parser_lines); $parser_i < $parser_iMax; $parser_i++) {
        ...
        switch ($row['storagetype']) {
            case 'html':
                if ($phpAds_config['gui_show_banner_html']) {
                    ...
                    $htmlcode = mb_strlen($htmlcode) > 500 ? mb_substr($htmlcode, 0, 500) . '...' : $htmlcode;

                    $htmlcode = chunk_preg_split($htmlcode, 65, "\n");

                    $htmlcode = str_replace("\n", "<br>\n", htmlspecialchars($htmlcode, ENT_QUOTES | ENT_HTML5));
                ...
                break;
            case 'network':
                if (preg_match("\[title\]([^\[]*)\[\/title\]", $row['htmltemplate'], $matches)) {
                    $title = $matches[1];
                }

                if (preg_match("\[logo\](.*)\[\/logo\]", $row['htmltemplate'], $matches)) {
                    $logo = $matches[1];
                }

                if (preg_match("\[login\](.*)\[\/login\]", $row['htmltemplate'], $matches)) {
                    $login = $matches[1];
                }
