...
function phpAds_PageHeader($ID, $extra = '')
{
    ...
    if (phpAds_Login != $ID && phpAds_Error != $ID) {
        // Prepare Navigation
        ...
        for ($i = 0; $i < count($sections) - 1; $i++) {
            ...
            [$filename, $title] = each($pages[(string)$sectionID]);
        ...
        if (isset($pages[(string)$ID]) && is_array($pages[(string)$ID])) {
            [$filename, $title] = each($pages[(string)$ID]);
        ...
        if (count($phpAds_context)) {
            ...
            for ($ci = 0, $ciMax = count($phpAds_context); $ci < $ciMax; $ci++) {
        ...
        if (count($phpAds_shortcuts)) {
            ...
            for ($si = 0, $siMax = count($phpAds_shortcuts); $si < $siMax; $si++) {
                $sidebar .= "<tr><td colspan='2'><img src='images/break.gif' height='1' width='160' vspace='4'></td></tr>";
    ...
    echo '<html' . ('ltr' != $phpAds_TextDirection ? " dir='" . $phpAds_TextDirection . "'" : '') . ">\n";
    ...
    if ($phpAds_showHelp) {
        echo "\t\t<script language='JavaScript' src='js-help.js'></script>\n";
    }
...
function phpAds_PageFooter()
{
    ...
    if ($phpAds_showHelp) {
        echo "<div id='helpLayer' name='helpLayer' style='position:absolute; left:" . ('ltr' != $phpAds_TextDirection ? '0' : '181') . "; top:-10; width:10px; height:10px; z-index:1; overflow: hidden; visibility: hidden;'>";
    ...
    if (!preg_match("/(index|maintenance-updates|install|upgrade)\.php$", $HTTP_SERVER_VARS['PHP_SELF'])) {
...
function phpAds_ShowSections($sections)
{
    ...
    for ($i = 0, $iMax = count($sections); $i < $iMax; $i++) {
        [$sectionUrl, $sectionStr] = each($pages[(string)$sections[$i]]);
...
function phpAds_sqlDie()
{
    ...
    if ('MySQL' == $phpAds_dbmsname) {
        $errornumber = phpAds_dbErrorNo();

        if (1027 == $errornumber || 1039 == $errornumber) {
            $corrupt = true;
        }

        if (1016 == $errornumber || 1030 == $errornumber) {
            // Probably corrupted table, do additional check

            eregi('[0-9]+', $error, $matches);

            if (126 == $matches[0] || 127 == $matches[0]
                || 132 == $matches[0] || 134 == $matches[0] || 135 == $matches[0] || 136 == $matches[0] || 141 == $matches[0] || 144 == $matches[0]
                || 145 == $matches[0]) {
                $corrupt = true;
            }
        }
