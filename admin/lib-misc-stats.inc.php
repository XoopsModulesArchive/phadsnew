...
echo "<option value='s'" . ('s' == $type ? ' selected' : '') . '>' . $strSizeDistribution . '</option>';
if (!$phpAds_config['compact_stats'] && $phpAds_config['geotracking_stats']) {
    echo "<option value='c'" . ('c' == $type ? ' selected' : '') . '>' . $strCountryDistribution . '</option>';
}
echo "<option value='t'" . ('t' == $type ? ' selected' : '') . '>' . $strTargetStats . '</option>';
...
'square' => $row_banners['width'] * $row_banners['height'],
    ...
    if ($phpAds_config['compact_stats']) {
        ...
        while (false !== ($row_stats = phpAds_dbFetchArray($res_stats))) {
            $dimension = $banners[$row_stats['bannerid']]['width'] . ' x ' . $banners[$row_stats['bannerid']]['height'];
    ...
    while (list($key, $value) = each($dimensions)) {
        ...
        $cpp = $value['square'] > 0 && $value['views'] > 0 ? $value['clicks'] / ($value['views'] * ($value['square'] / (468 * 60))) * 100 : 0;
...
if ('c' == $type) {
    ...
    while (list($key, $value) = each($countries)) {
        ...
        $flag = ('' == $key || 'A1' == $key || 'A2' == $ket) ? 'unknown' : mb_strtolower($key);
