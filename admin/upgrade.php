...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    if (3 == $step && isset($retry)) {
        $step = 2;
    }

    if (3 == $step && isset($ignore)) {
        $step = 4;
    }

    // Check privileges and writability of config file

    if ($upgrade && (1 == $step || 2 == $step)) {
        ...
        if (isset($phpAds_config['delivery_caching']) && 'file' == $phpAds_config['delivery_caching']) {
    ...
    if ($upgrade && 1 == $step) {
        ...
        if (!phpAds_databaseUpgradeSupported) {
            $message .= '<br><br><b>' . $strCantUpdateDB . '</b>';
        }
        ...
        $table_type = $phpAds_config['table_type'] ?? '';
        ...
        [$banners, $priority_sum] = phpAds_dbFetchRow(
            phpAds_dbQuery(
                '
			SELECT
				COUNT(bannerid),
