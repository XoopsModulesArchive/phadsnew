...
if ('english' != $phpAds_config['language'] && file_exists(phpAds_path . '/language/' . $phpAds_config['language'] . '/maintenance.lang.php')) {
...
function phpAds_MaintenanceSelection($section)
{
    ...
    echo "<option value='banners'" . ('banners' == $section ? ' selected' : '') . '>' . $strBanners . '</option>';

    echo "<option value='priority'" . ('priority' == $section ? ' selected' : '') . '>' . $strPriority . '</option>';

    if ($phpAds_config['compact_stats'] && ($verboseviews > 0 || $verboseclicks > 0)) {
        echo "<option value='stats'" . ('stats' == $section ? ' selected' : '') . '>' . $strStats . '</option>';
    }

    if (true === $phpAds_config['type_web_allow']
        && ((0 == $phpAds_config['type_web_mode']
             && '' != $phpAds_config['type_web_dir']) || (1 == $phpAds_config['type_web_mode']
                && '' != $phpAds_config['type_web_ftp']))
        && '' != $phpAds_config['type_web_url']) {
        echo "<option value='storage'" . ('storage' == $section ? ' selected' : '') . '>' . $strStorage . '</option>';
    }

    if ('none' != $phpAds_config['delivery_caching']) {
        echo "<option value='cache'" . ('zones' == $section ? ' selected' : '') . '>' . $strCache . '</option>';
    }
