...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    phpAds_ShowSections(['2.4.1', '2.4.2']);
} else {
    ...
    if ($phpAds_config['client_welcome']) {
        ...
        if (!empty($phpAds_client_welcome_msg)) {
            ...
            require __DIR__ . '/templates/welcome-publisher.html';
        }
...
if (isset($zones) && is_array($zones) && count($zones) > 0) {
    ...
    for (reset($zones); $key = key($zones); next($zones)) {
        ...
        if (isset($zones[$key]['banners']) && count($zones[$key]['banners']) > 0) {
...
if (!isset($zones) || !is_array($zones) || 0 == count($zones)) {
    ...
    for (reset($zones); $key = key($zones); next($zones)) {
        ...
        if (isset($zone['banners'])) {
            if (isset($zone['expand']) && '1' == $zone['expand']) {
        ...
        } elseif (phpAds_ZoneInterstitial == $zone['delivery']) {
            echo "<img src='images/icon-interstitial.gif' align='absmiddle'>&nbsp;";
        } elseif (phpAds_ZonePopup == $zone['delivery']) {
        ...
        if (isset($zone['banners']) && count($zone['banners']) > 0 && isset($zone['expand']) && '1' == $zone['expand']) {
            ...
            while (list($bkey, ) = each($banners)) {
                ...
                if (preg_match('bannerid:' . $banners[$bkey]['bannerid'], $zone['what'])) {
