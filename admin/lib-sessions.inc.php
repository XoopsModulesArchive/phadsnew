...
function phpAds_SessionDataFetch()
{
    ...
    if (isset($HTTP_COOKIE_VARS['sessionID']) && '' != $HTTP_COOKIE_VARS['sessionID']) {
        $result = phpAds_dbQuery(
            'SELECT sessiondata FROM ' . $phpAds_config['tbl_session'] . " WHERE sessionid='" . $HTTP_COOKIE_VARS['sessionID'] . "'" . ' AND UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(lastused) < 3600'
        );
...
function phpAds_SessionDataStore()
{
    ...
    if (isset($HTTP_COOKIE_VARS['sessionID']) && '' != $HTTP_COOKIE_VARS['sessionID']) {
        phpAds_dbQuery(
            'REPLACE INTO ' . $phpAds_config['tbl_session'] . " VALUES ('" . $HTTP_COOKIE_VARS['sessionID'] . "', '" . addslashes(serialize($Session)) . "', null )"
        );
    }
