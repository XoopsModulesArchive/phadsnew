...
if (isset($row['span']) && $row['span'] > 0) {
    ...
    if ($phpAds_config['compact_stats']) {
        ...
        while (false !== ($row = phpAds_dbFetchArray($result))) {
            if (!in_array($row['source'], $sources, true)) {
                $sources[] = $row['source'];
            }
        }
    }
    ...
    if ((1 == count($sources) && '' != $sources[0]) || count($sources) > 1) {
        echo '&nbsp;&nbsp;';
    ...
    if ('m' == $period || 'd' == $period) {
        ...
        for ($d = 0; $d < $limit; $d++) {
            ...
            if (isset($lib_history_hourlyurl) && 'd' == $period && $available) {
        ...
        echo "<td align='"
             . $phpAds_TextAlignRight
             . "' height='25'>("
             . number_format(($totals['clicks'] ? $totalclicks / $totals['clicks'] * 100 : 0), $phpAds_config['percentage_decimals'], $phpAds_DecimalPoint, $phpAds_ThousandsSeperator)
             . '%)&nbsp;&nbsp;'
             . phpAds_formatNumber($totalclicks)
             . '</td>';
        ...
        for ($d = 0; $d < $limit; $d++) {
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                if (isset($stats[$key])) {
                    ...
                    $clicks[$i] = $stats[$key]['sum_clicks'] ?? 0;
