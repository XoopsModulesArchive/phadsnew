...
if (isset($convert)) {
    ...
    if ('sql' == $row['storagetype'] || 'web' == $row['storagetype']) {
    ...
    if ($swf_file) {
        if (phpAds_SWFVersion($swf_file) >= 3
            ...
            [$result, $parameters] = phpAds_SWFConvert($swf_file, $compress, $convert_links);
...
if ($result) {
    ...
    while (list($key, $val) = each($result)) {
        ...
        echo "<input type='radio' name='chosen_link' value='" . $key . "'" . (0 == $i ? ' checked' : '') . '></td></tr>';
