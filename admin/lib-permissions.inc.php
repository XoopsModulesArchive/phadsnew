...
'zone' => 'affiliate-zones.php',
    ...
    if ('edit' == $part[1] || 'index' == $part[1]) {
        if ('client' == $part[0]) {
            $part[0] = '';
        } elseif ('campaign' == $part[0]) {
            $part[0] = 'client';
        } elseif ('banner' == $part[0]) {
            $part[0] = 'campaign';
        } elseif ('affiliate' == $part[0]) {
            $part[0] = '';
        } elseif ('zone' == $part[0]) {
            $part[0] = 'affiliate';
        }
        ...
        case 'banner':
            if (!is_numeric($bannerid)) {
                if (is_numeric($clientid) && is_numeric($campaignid)) {
                    ...
                    if ('activate' == $part[1] || 'delete' == $part[1]) {
                        break;
                    }
            ...
            } elseif (isset($part[1]) && 'htmlpreview' == $part[1]) {
                break;
            }
...
function phpAds_Login()
{
    ...
    if (phpAds_SuppliedCredentials()) {
        ...
        if ($HTTP_COOKIE_VARS['sessionID'] != $_POST['phpAds_cookiecheck']) {
            $HTTP_COOKIE_VARS['sessionID'] = phpAds_SessionStart();
...
function phpAds_IsLoggedIn()
{
    ...
    return (isset($Session['loggedin']) ? ('t' == $Session['loggedin']) : false);
}
...
function phpAds_LoginScreen($message = '', $sessionID = 0)
{
    ...
    if (true === $phpAds_config['ui_enabled']) {
        echo '<br>';
