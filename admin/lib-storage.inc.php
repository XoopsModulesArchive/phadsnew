...
function phpAds_ImageStore($storagetype, $name, $buffer, $overwrite = false)
{
    ...
    if ('web' == $storagetype) {
        if (0 == $phpAds_config['type_web_mode']) {
            ...
            if ('' != $server['path'] && '/' == mb_substr($server['path'], 0, 1)) {
                $server['path'] = mb_substr($server['path'], 1);
            }

            if ('ftp' == $server['scheme']) {
...
function phpAds_ImageRetrieve($storagetype, $name)
{
    ...
    if ('web' == $storagetype) {
        if (0 == $phpAds_config['type_web_mode']) {
            ...
            $result = @fread(
                @fopen($phpAds_config['type_web_dir'] . '/' . $name, 'rb'),
...
function phpAds_SqlUniqueName($name)
{
    ...
    $extension = mb_substr($name, mb_strrpos($name, '.') + 1);

    $base = mb_substr($name, 0, mb_strrpos($name, '.'));
...
function phpAds_LocalUniqueName($name)
{
    ...
    if (false === @file_exists($phpAds_config['type_web_dir'] . '/' . $base . '.' . $extension)) {
        return ($base . '.' . $extension);
    }
    ...
    while (false === $found) {
        ...
        if (false === @file_exists($phpAds_config['type_web_dir'] . '/' . $base . '_' . $i . '.' . $extension)) {
            $found = true;
        }
    }
