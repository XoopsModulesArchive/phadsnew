...
if (!isset($compact)) {
    $compact = $Session['prefs']['stats-campaign-banners.php']['compact'] ?? 't';
}

if (!isset($view)) {
    $view = $Session['prefs']['stats-campaign-banners.php']['view'] ?? 'all';
}

if (!isset($listorder)) {
    $listorder = $Session['prefs']['stats-campaign-banners.php']['listorder'] ?? 'name';
}
...
if (phpAds_isUser(phpAds_Client)) {
    if (phpAds_getUserID() == phpAds_getParentID($campaignid)) {
        ...
        phpAds_ShowSections(['1.2.1', '1.2.2', '1.2.3']);
    } else {
        phpAds_PageHeader('1');
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    phpAds_ShowSections(['2.1.2.1', '2.1.2.2', '2.1.2.3']);
}
...
if (count($order_array) > 0) {
    ...
    while (list($bannerid, ) = each($order_array)) {
        ...
        if (0 != $adclicks && 'adclicks' == $view) {
            continue;
        }    // Don't show banners without adclicks

        if (0 != $adviews && 'adviews' == $view) {
            continue;
        }    // Don't show banners without adclicks
        ...
        if (isset($compact) && 't' != $compact) {
            ...
            if ('t' == $row_banners['active']) {
                if ('html' == $row_banners['storagetype']) {
                    echo "<img src='images/icon-banner-html.gif' align='absmiddle'>";
                } elseif ('txt' == $row_banners['storagetype']) {
            ...
            if (phpAds_isUser(phpAds_Client) && phpAds_isAllowed(phpAds_DisableBanner) && 't' == $row_banners['active']) { // only for the client if allowed
                echo '&nbsp;&nbsp;&nbsp;&nbsp;';
            ...
            if (phpAds_isUser(phpAds_Client) && phpAds_isAllowed(phpAds_ActivateBanner) && 't' != $row_banners['active']) { // only for the client if allowed
                echo '&nbsp;&nbsp;&nbsp;&nbsp;';
            ...
            */
            ...
            if (preg_match('Mozilla/6', $HTTP_SERVER_VARS['HTTP_USER_AGENT']) || preg_match('IE', $HTTP_SERVER_VARS['HTTP_USER_AGENT'])) {
    ...
    echo "<option value='all'" . ('all' == $view ? ' selected' : '') . ">$strShowAllBanners</option>";

    echo "<option value='adclicks'" . ('adclicks' == $view ? ' selected' : '') . ">$strShowBannersNoAdClicks</option>";

    echo "<option value='adviews'" . ('adviews' == $view ? ' selected' : '') . ">$strShowBannersNoAdViews</option>";
    ...
    echo "<option value='f'" . ('t' != $compact ? ' selected' : '') . ">$strVerbose</option>";

    echo "<option value='t'" . ('t' == $compact ? ' selected' : '') . ">$strCompact</option>";
    ...
    if ('none' != phpAds_GDImageFormat() && $totaladviews > 0 && !$phpAds_config['compact_stats']) {
    ...
    [$desc, $enddate, $daysleft] = days_left($campaignid);
