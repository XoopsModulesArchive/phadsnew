...
phpAds_ShowSections(['2.1', '2.4', '2.2', '2.5']);
...
if (!isset($hideinactive)) {
    $hideinactive = $Session['prefs']['stats-global-client.php']['hideinactive'] ?? ('t' == $phpAds_config['gui_hide_inactive']);
}
...
// Check period range
if (0 == $period_range['start_month'] || 0 == $period_range['start_day'] || 0 == $period_range['start_year']) {
    $period_begin = 0;
...
if (0 == $period_range['end_month'] || 0 == $period_range['end_day'] || 0 == $period_range['end_year']) {
...
if (!$phpAds_config['compact_stats']) {
    switch ($period) {
        case 'r':
            $limit = ' AND t_stamp >= ' . date('YmdHis', $period_begin) . ' AND t_stamp < ' . date('YmdHis', $period_end);
            break;
        case 'y':
            $timestamp_begin = mktime(0, 0, 0, date('m'), date('d') - 1, date('Y'));
        ...
        case 't':
            ...
            break;
        case 'w':
            $timestamp_begin = mktime(0, 0, 0, date('m'), date('d') - 6, date('Y'));
            $limit = ' AND t_stamp >= ' . date('YmdHis', $timestamp_begin);
            break;
        case 'm':
            $timestamp_begin = mktime(0, 0, 0, date('m'), 1, date('Y'));
            $limit = ' AND t_stamp >= ' . date('YmdHis', $timestamp_begin);
            break;
        default:
            $limit = '';
    ...
    switch ($period) {
        case 'r':
            $limit = ' AND day >= ' . date('Ymd', $period_begin) . ' AND day < ' . date('Ymd', $period_end);
            break;
        case 'y':
            $timestamp_begin = mktime(0, 0, 0, date('m'), date('d') - 1, date('Y'));
            $timestamp_end = mktime(0, 0, 0, date('m'), date('d'), date('Y'));
...
if (isset($expand) && '' != $expand) {
    switch ($expand) {
        case 'all':
            $node_array = [];
            if (isset($clients)) {
                while (list($key, ) = each($clients)) {
                    $node_array[] = $key;
                }
            }
            if (isset($campaigns)) {
                while (list($key, ) = each($campaigns)) {
                    $node_array[] = $key;
                }
            }
            break;
...
$node_array_size = count($node_array);
...
if (isset($banners) && is_array($banners) && count($banners) > 0) {
    ...
    while (list($bkey, $banner) = each($banners)) {
        if (false === $hideinactive || 't' == $banner['active']) {
...
if (isset($campaigns) && is_array($campaigns) && count($campaigns) > 0) {
    ...
    while (list($ckey, $campaign) = each($campaigns)) {
        ...
        if (false === $hideinactive
            || 't' == $campaign['active']
               && (0 != count($campaign['banners']) || count($campaign['banners']) == $campaign['count'])) {
...
if (isset($clients) && is_array($clients) && count($clients) > 0) {
    ...
    while (list($key, $client) = each($clients)) {
        ...
        if (0 == count($client['campaigns']) && $client['hideinactive'] > 0) {
            $clientshidden++;
...
if (isset($clients) && is_array($clients) && count($clients) > 0) {
    ...
    for (reset($clients); $key = key($clients); next($clients)) {
        ...
        if (isset($clients[$key]['campaigns']) && count($clients[$key]['campaigns']) > 0) {
            ...
            for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
                ...
                if (isset($campaigns[$ckey]['banners']) && count($campaigns[$ckey]['banners']) > 0) {
...
echo "<option value='t'" . ('t' == $period ? ' selected' : '') . '>' . $strCollectedToday . '</option>';
echo "<option value='y'" . ('y' == $period ? ' selected' : '') . '>' . $strCollectedYesterday . '</option>';
echo "<option value='w'" . ('w' == $period ? ' selected' : '') . '>' . $strCollected7Days . '</option>';
echo "<option value='m'" . ('m' == $period ? ' selected' : '') . '>' . $strCollectedMonth . '</option>';
echo "<option value='' disabled>-----------------------------------------</option>";
echo "<option value='r'" . ('r' == $period ? ' selected' : '') . '>' . $strCollectedRange . '</option>';
echo '</select>';
...
if ('r' == $period) {
    ...
    echo "<option value='0'" . (0 == $period_range['start_day'] ? ' selected' : '') . ">-</option>\n";
    ...
    echo "<option value='0'" . (0 == $period_range['start_month'] ? ' selected' : '') . ">-</option>\n";
    ...
    echo "<option value='0'" . (0 == $period_range['start_year'] ? ' selected' : '') . ">-</option>\n";
    ...
    echo "<option value='0'" . (0 == $period_range['end_day'] ? ' selected' : '') . ">-</option>\n";
    ...
    echo "<option value='0'" . (0 == $period_range['end_month'] ? ' selected' : '') . ">-</option>\n";
    ...
    echo "<option value='0'" . (0 == $period_range['end_year'] ? ' selected' : '') . ">-</option>\n";
...
if ($clientshidden > 0 || $totalviews > 0 || $totalclicks > 0) {
    ...
    for (reset($clients); $key = key($clients); next($clients)) {
        ...
        if (isset($client['campaigns']) && count($client['campaigns']) > 0 && '1' == $client['expand']) {
            ...
            for (reset($campaigns); $ckey = key($campaigns); next($campaigns)) {
                ...
                echo "<tr height='25' " . (0 == $i % 2 ? "bgcolor='#F6F6F6'" : '') . "><td height='25'>";

                echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';

                if (isset($campaigns[$ckey]['banners'])) {
                    if ('1' == $campaigns[$ckey]['expand']) {
                ...
                if ('1' == $campaigns[$ckey]['expand'] && isset($campaigns[$ckey]['banners'])) {
                    $banners = $campaigns[$ckey]['banners'];
                    ...
                    for (reset($banners); $bkey = key($banners); next($banners)) {
                        ...
                        if ('t' == $banners[$bkey]['active'] && 't' == $campaigns[$ckey]['active']) {
                            if ('html' == $banners[$bkey]['storagetype']) {
                                echo "<img src='images/icon-banner-html.gif' align='absmiddle'>";
                            } elseif ('txt' == $banners[$bkey]['storagetype']) {
                                echo "<img src='images/icon-banner-text.gif' align='absmiddle'>";
                            } elseif ('url' == $banners[$bkey]['storagetype']) {
        ...
        if (isset($client['banners']) && count($client['banners']) > 0) {
            // Divider
    ...
    echo "<tr height='25' " . (0 == $i % 2 ? "bgcolor='#F6F6F6'" : '') . "><td height='25'>&nbsp;&nbsp;<b>" . $strTotal . '</b></td>';
