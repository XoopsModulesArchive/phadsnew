...
function adviews_left($clientid)
{
    ...
    if (1 == phpAds_dbNumRows($res_client)) {
...
function days_left($clientid)
{
    ...
    if (1 == phpAds_dbNumRows($res_client)) {
        ...
        if ('0000-00-00' != $row_client['expire'] && '' != $row_client['expire']) {
            $expiration[] = [
...
'absolute' => true,
        ...
        if (-1 != $row_client['views']) {
            ...
            if (1 == phpAds_dbNumRows($res_views)) {
                $row_views = phpAds_dbFetchArray($res_views);

                if (!isset($row_views['days_since_start'])
                    || '' == $row_views['days_since_start'] || 0 == $row_views['days_since_start']
                    || null === $row_views['days_since_start']) {
...
'absolute' => true,
        ...
        if (-1 != $row_client['clicks']) {
            ...
            if (1 == phpAds_dbNumRows($res_clicks)) {
                $row_clicks = phpAds_dbFetchArray($res_clicks);

                if (!isset($row_clicks['days_since_start'])
                    || '' == $row_clicks['days_since_start'] || 0 == $row_clicks['days_since_start']
                    || null === $row_clicks['days_since_start']) {
...
'absolute' => true,
    ...
    if (isset($expiration) && count($expiration) > 0) {
        $sooner = $expiration[0];

        for ($i = 0, $iMax = count($expiration); $i < $iMax; $i++) {
