...
function phpAds_getVerbose($base, $count)
{
    ...
    while (false !== ($row = phpAds_dbFetchArray($result))) {
        $stats["'" . $row['bannerid'] . "'"]["'" . $row['zoneid'] . "'"]["'" . $row['hour'] . "'"]["'" . $row['source'] . "'"] = ['views' => $row['qnt'], 'clicks' => 0];
    }
    ...
    while (false !== ($row = phpAds_dbFetchArray($result))) {
        if (isset($stats["'" . $row['bannerid'] . "'"]["'" . $row['zoneid'] . "'"]["'" . $row['hour'] . "'"]["'" . $row['source'] . "'"])) {
            ...
            $stats["'" . $row['bannerid'] . "'"]["'" . $row['zoneid'] . "'"]["'" . $row['hour'] . "'"]["'" . $row['source'] . "'"] = ['views' => 0, 'clicks' => $row['qnt']];
        }
    }
        ...
        for (reset($stats_b); $zoneid = key($stats_b); next($stats_b)) {
            ...
            for (reset($stats_z); $hour = key($stats_z); next($stats_z)) {
                ...
                for (reset($stats_h); $source = key($stats_h); next($stats_h)) {
                    ...
                    $result = phpAds_dbQuery(
                        'INSERT INTO ' . $phpAds_config['tbl_adstats'] . ' SET clicks = ' . $stats_s['clicks'] . ', views = ' . $stats_s['views'] . ", 
...
if (isset($action) && 'start' == $action) {
    ...
    $base = mktime(0, 0, 0, mb_substr($min, 4, 2), mb_substr($min, 6, 2), mb_substr($min, 0, 4));
