...
if (isset($submit)) {
    ...
    if (phpAds_ZoneText == $delivery) {
        ...
        if ('custom' == $sizetype) {
            if (isset($zwidth) && '*' == $zwidth) {
                $zwidth = -1;
            }

            if (isset($zheight) && '*' == $zheight) {
                $zheight = -1;
            }
        } else {
            [$zwidth, $zheight] = explode('x', $size);
        }
    }
    ...
    if (isset($zoneid) && '' != $zoneid) {
        $res = phpAds_dbQuery(
			...
			SET
				...
				height='" . $zheight . "',
				delivery='" . $delivery . "'
				" . (phpAds_ZoneText != $delivery && phpAds_ZoneBanner != $delivery ? ", append = ''" : '') . '
				' . (phpAds_ZoneText != $delivery ? ", prepend = ''" : '') . "
			WHERE
				zoneid='" . $zoneid . "'
			"
        ) or phpAds_sqlDie();
...
echo "<tr><td><input type='radio' name='delivery' value='" . phpAds_ZoneBanner . "'" . (phpAds_ZoneBanner == $zone['delivery'] ? ' CHECKED' : '') . " onClick='phpAds_formEnableSize();' tabindex='" . ($tabindex++) . "'>";
echo "&nbsp;<img src='images/icon-zone.gif' align='absmiddle'>&nbsp;" . $strBannerButtonRectangle . '</td></tr>';

if ($phpAds_config['allow_invocation_interstitial'] || phpAds_ZoneInterstitial == $zone['delivery']) {
    echo "<tr><td><input type='radio' name='delivery' value='" . phpAds_ZoneInterstitial . "'" . (phpAds_ZoneInterstitial == $zone['delivery'] ? ' CHECKED' : '') . " onClick='phpAds_formEnableSize();' tabindex='" . ($tabindex++) . "'>";

    echo "&nbsp;<img src='images/icon-interstitial.gif' align='absmiddle'>&nbsp;" . $strInterstitial . '</td></tr>';
}

if ($phpAds_config['allow_invocation_popup'] || phpAds_ZonePopup == $zone['delivery']) {
    echo "<tr><td><input type='radio' name='delivery' value='" . phpAds_ZonePopup . "'" . (phpAds_ZonePopup == $zone['delivery'] ? ' CHECKED' : '') . " onClick='phpAds_formEnableSize();' tabindex='" . ($tabindex++) . "'>";

    echo "&nbsp;<img src='images/icon-popup.gif' align='absmiddle'>&nbsp;" . $strPopup . '</td></tr>';
}

echo "<tr><td><input type='radio' name='delivery' value='" . phpAds_ZoneText . "'" . (phpAds_ZoneText == $zone['delivery'] ? ' CHECKED' : '') . " onClick='phpAds_formDisableSize();' tabindex='" . ($tabindex++) . "'>";
...
echo $strWidth . ": <input class='flat' size='5' type='text' name='zwidth' value='" . ($zone['width'] ?? '') . "'" . $sizedisabled . " onkeydown='phpAds_formEditSize()' onBlur='phpAds_formUpdate(this);' tabindex='" . ($tabindex++) . "'>";
echo '&nbsp;&nbsp;&nbsp;';
echo $strHeight . ": <input class='flat' size='5' type='text' name='zheight' value='" . ($zone['height'] ?? '') . "'" . $sizedisabled . " onkeydown='phpAds_formEditSize()' onBlur='phpAds_formUpdate(this);' tabindex='" . ($tabindex++) . "'>";
echo '</td></tr></table>';
echo '</td></tr>';
