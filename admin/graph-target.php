...
if ('d' == $period || 'm' == $period) {
    for ($d = 0; $d < $limit; $d++) {
        ...
        if (isset($stats[$key])) {
            $items[$d]['value1'] = $stats[$key]['sum_target'] ?? 0;

            $items[$d]['value2'] = $stats[$key]['sum_views'] ?? 0;
...
if ('w' == $period) {
    ...
    for ($d = 0; $d < $limit; $d++) {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            if (isset($stats[$key])) {
                $totalweekviews += $stats[$key]['sum_views'] ?? 0;

                $totalweektarget += $stats[$key]['sum_target'] ?? 0;
...
require __DIR__ . '/lib-graph.inc.php';
