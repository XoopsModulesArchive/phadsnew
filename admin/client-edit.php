...
if (isset($submit)) {
    ...
    if ('' == $clientreportlastdate || '0000-00-00' == $clientreportlastdate
    ...
    if (phpAds_isUser(phpAds_Admin)) {
        ...
        if (isset($clientusername) && '' != $clientusername) {
            $res = phpAds_dbQuery(
				...
				WHERE
					LOWER(username) = '" . mb_strtolower($clientusername) . "'
			"
            ) or phpAds_sqlDie();

            $duplicateaffiliate = (phpAds_dbNumRows($res) > 0);

            $duplicateadmin = (mb_strtolower($phpAds_config['admin']) == mb_strtolower($clientusername));
            ...
            if ('' == $clientid) {
                $res = phpAds_dbQuery(
					...
					FROM
						' . $phpAds_config['tbl_clients'] . "
					WHERE
						LOWER(clientusername) = '" . mb_strtolower($clientusername) . "'
				"
                ) or phpAds_sqlDie();
                ...
                $res = phpAds_dbQuery(
					...
					FROM
						' . $phpAds_config['tbl_clients'] . "
					WHERE
						LOWER(clientusername) = '" . mb_strtolower($clientusername) . "' AND
						clientid != '$clientid'
					"
                ) or phpAds_sqlDie();
        ...
        if (isset($clientpermissions) && is_array($clientpermissions)) {
            for ($i = 0, $iMax = count($clientpermissions); $i < $iMax; $i++) {
    ...
    */
    ...
    if (0 == count($errormessage)) {
        if (!isset($clientid) || '' == $clientid) {
            ...
            phpAds_dbQuery($query) or phpAds_sqlDie();
...
// Reports
echo "<input type='hidden' name='clientreportlastdate' value='" . ($client['reportlastdate'] ?? '') . "'>";
echo "<input type='hidden' name='clientreportprevious' value='" . ($client['report'] ?? '') . "'>";

echo "<tr><td width='30'>&nbsp;</td><td colspan='2'>";
echo "<input type='checkbox' name='clientreportdeactivate' value='t'" . ('t' == $client['reportdeactivate'] ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;";
echo $strSendDeactivationWarning;
echo '</td></tr>';

// Interval
echo "<tr><td width='30'>&nbsp;</td><td colspan='2'>";
echo "<input type='checkbox' name='clientreport' value='t'" . ('t' == $client['report'] ? ' CHECKED' : '') . " tabindex='" . ($tabindex++) . "'>&nbsp;";
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    echo '<td>' . ($client['clientusername'] ?? '') . '</td>';
}
...
// Get unique username
$unique_users = [$phpAds_config['admin']];
