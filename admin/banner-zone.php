...
if (isset($submit)) {
    ...
    for (reset($previouszone); $key = key($previouszone); next($previouszone)) {
        if ((true === $previouszone[$key] && (!isset($includezone[$key]) || 't' != $includezone[$key]))
            || (true !== $previouszone[$key] && (isset($includezone[$key]) && 't' == $includezone[$key]))) {
    ...
    for (reset($affiliates); $akey = key($affiliates); next($affiliates)) {
        ...
        if (isset($affiliate['zones'])) {
            ...
            for (reset($zones); $zkey = key($zones); next($zones)) {
                ...
                echo '<td ' . (0 == $i % 2 ? "bgcolor='#F6F6F6'" : '') . "><img src='images/spacer.gif' height=1'></td>";
                ...
                if (phpAds_ZoneBanners == $zone['zonetype']) {
                    if ($zone['linked']) {
