...
if (isset($submit)) {
    ...
    if (isset($HTTP_POST_FILES['upload'])) {
        if ('' != $HTTP_POST_FILES['upload']['name']
            && 'none' != $HTTP_POST_FILES['upload']['tmp_name']) {
    ...
    switch ($storagetype) {
        case 'sql':
            if (isset($uploaded) && 't' == $replaceimage) {
                // Get the filename
                ...
                $ext = mb_substr($uploaded['name'], mb_strrpos($uploaded['name'], '.') + 1);

                switch (mb_strtolower($ext)) {
                    case 'jpeg':
                        $final['contenttype'] = 'jpeg';
                        break;
                    case 'jpg':
                        $final['contenttype'] = 'jpeg';
                        break;
                    case 'html':
                        ...
                        break;
                    case 'png':
                        $final['contenttype'] = 'png';
                        break;
                    case 'gif':
                        $final['contenttype'] = 'gif';
                        break;
                    case 'swf':
            ...
            if (!isset($bannerid) || '0' == $bannerid || '' == $bannerid) {
            ...
            if (isset($alink) && is_array($alink) && count($alink)) {
                while (list($key, $val) = each($alink)) {
                    if ('http://' == mb_substr($val, 0, 7) && mb_strlen($val) > 7) {
        ...
        case 'web':
            if (isset($uploaded) && 't' == $replaceimage) {
                ...
                if (false === $final['filename']) {
                    phpAds_PageHeader('1');

                    if (0 == $phpAds_config['type_web_mode']) {
    ...
    if (phpAds_isUser(phpAds_Admin)) {
        if (isset($keyword) && '' != $keyword) {
            $keywordArray = preg_split('[ ,]+', trim($keyword));
    ...
    if (isset($bannerid) && '' != trim($bannerid)) {
        ...
        $values_fields = preg_replace(', $', '', $values_fields);
...
if ('' != $bannerid) {
    ...
    while (false !== ($others = phpAds_dbFetchArray($res))) {
        phpAds_PageContext(
    ...
    if (phpAds_isUser(phpAds_Admin)) {
        ...
        phpAds_ShowSections(['4.1.3.4.2', '4.1.3.4.3', '4.1.3.4.6', '4.1.3.4.4']);
    } else {
        ...
        phpAds_ShowSections(['1.2.2.1', '1.2.2.2']);
    }
    ...
    if ('swf' == $row['contenttype']) {
        if (false !== mb_strpos($row['htmltemplate'], 'alink1={targeturl:')) {
            $buffer = $row['htmltemplate'];

            while (eregi("alink([0-9]+)=\{targeturl:([^\}]+)\}", $buffer, $regs)) {
                if (false !== mb_strpos($regs[2], '|source:')) {
                    [$hardcoded_links[$regs[1]], $hardcoded_sources[$regs[1]]] = explode('|source:', $regs[2]);
                } else {
...
if (isset($storagetype) && 'sql' == $storagetype) {
    $show_sql = true;
}
if (isset($storagetype) && 'web' == $storagetype) {
    $show_web = true;
}
if (isset($storagetype) && 'url' == $storagetype) {
    $show_url = true;
}
if (isset($storagetype) && 'html' == $storagetype) {
    $show_html = true;
}
if (isset($storagetype) && 'txt' == $storagetype) {
    $show_txt = true;
}
if (isset($storagetype) && 'network' == $storagetype) {
    $show_network = true;
}
...
if (!isset($bannerid) || '' == $bannerid) {
    ...
    if ($show_sql) {
        echo "<option value='sql'" . ('sql' == $storagetype ? ' selected' : '') . '>' . $strMySQLBanner . '</option>';
    }

    if ($show_web) {
        echo "<option value='web'" . ('web' == $storagetype ? ' selected' : '') . '>' . $strWebBanner . '</option>';
    }

    if ($show_url) {
        echo "<option value='url'" . ('url' == $storagetype ? ' selected' : '') . '>' . $strURLBanner . '</option>';
    }

    if ($show_html) {
        echo "<option value='html'" . ('html' == $storagetype ? ' selected' : '') . '>' . $strHTMLBanner . '</option>';
    }

    if ($show_txt) {
        echo "<option value='txt'" . ('txt' == $storagetype ? ' selected' : '') . '>' . $strTextBanner . '</option>';
    }

    if ($show_network) {
        echo "<option value='network'" . ('network' == $storagetype ? ' selected' : '') . '>' . $strBannerNetwork . '</option>';
    }
...
if ('sql' == $storagetype) {
    ...
    if (isset($row['filename']) && '' != $row['filename']) {
        ...
        switch ($row['contenttype']) {
            case 'swf':
                echo "<img src='images/icon-filetype-swf.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'dcr':
                echo "<img src='images/icon-filetype-swf.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'jpeg':
                echo "<img src='images/icon-filetype-jpg.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'gif':
                echo "<img src='images/icon-filetype-gif.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'png':
                echo "<img src='images/icon-filetype-png.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'rpm':
                echo "<img src='images/icon-filetype-rpm.gif' align='absmiddle'> " . $row['filename'];
                break;
            case 'mov':
                echo "<img src='images/icon-filetype-mov.gif' align='absmiddle'> " . $row['filename'];
                break;
            default:
                echo "<img src='images/icon-banner-stored.gif' align='absmiddle'> " . $row['filename'];
                break;
        }

        $size = phpAds_ImageSize($storagetype, $row['filename']);

        if (0 == round($size / 1024)) {
...
if ('html' == $storagetype) {
    ...
    echo "' tabindex='" . ($tabindex++) . "'>" . htmlentities($row['htmltemplate'], ENT_QUOTES | ENT_HTML5) . '</textarea></td></tr>';

    echo "<tr><td width='30'>&nbsp;</td>";

    echo "<td colspan='2'><input type='checkbox' name='autohtml' value='t'" . (isset($row['autohtml']) && 't' == $row['autohtml'] ? ' checked' : '') . " tabindex='" . ($tabindex++) . "'> " . $strAutoChangeHTML . '</td></tr>';
...
if ('network' == $storagetype) {
    ...
    if (preg_match("\[define-vars\](.*)\[\/define-vars\]", $networkinfo, $matches)) {
        $vars = $matches[1];
    }
    ...
    for ($i = 0, $iMax = count($result['vars']); $i < $iMax; $i++) {
        echo "<tr><td width='30'>&nbsp;</td>";
...
if (phpAds_isUser(phpAds_Admin)) {
    ...
    echo "<td><input class='flat' size='6' type='text' name='weight' value='" . ($row['weight'] ?? $phpAds_config['default_banner_weight']) . "' tabindex='" . ($tabindex++) . "'></td></tr>";
