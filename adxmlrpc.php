...
function phpAds_xmlrpcView($msg)
{
    ...
    $params = [
        ['name' => 'remote_info', 'scalar' => false, 'type' => 'struct'],
        ['name' => 'what', 'scalar' => true, 'type' => 'string'],
        ['name' => 'clientid', 'scalar' => true, 'type' => 'int'],
        ['name' => 'target', 'scalar' => true, 'type' => 'string'],
        ['name' => 'source', 'scalar' => true, 'type' => 'string'],
        ['name' => 'withtext', 'scalar' => true, 'type' => 'boolean'],
        ...
        if ('remote_info' == $params[$i]['name']) {
            ...
            while (list($k, $v) = each($p)) {
                switch ($k) {
                    ...
                    case 'remote_host':
                        ...
                        break;
                    default:
                        $varname = 'HTTP_' . mb_strtoupper($k);
                        $GLOBALS['HTTP_SERVER_VARS'][$varname] = $GLOBALS[$varname] = $v;
                        break;
                }
            }
        } elseif ('scalar' == $p->kindOf()) {
            ...
            switch ($params[$i]['type']) {
                ...
                case 'boolean':
                    ...
                    break;
                default:
                    $view_params[] = "'" . addcslashes($p, "\0..\37'") . "'";
                    break;
            }
        } else {
    ...
    eval('$output = view_raw(' . implode(', ', $view_params) . ');');
    ...
    return $GLOBALS['phpAds_xmlError'] ?? new xmlrpcresp(phpAds_xmlrpcEncode($output));
}
...
function phpAds_xmlrpcErrorHandler($errno, $errstr, $errfile, $errline)
{
    ...
    if ($errno & (E_ERROR | E_USER_ERROR)) {
        $phpAds_xmlError = new xmlrpcresp(
            0,
            $xmlrpcerruser + 100,
...
if (function_exists('set_errorHandler')) {
