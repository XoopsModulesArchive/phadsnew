    ...
    switch ($row['storagetype']) {
        case 'url':
            ...
            if (preg_match('#\{random(:([0-9]+)){0,1}\}#i', $row['imageurl'], $matches)) {
                ...
                for ($r = 0; $r < $lastdigits; $r += 9) {
                    $lastrandom .= (string)random_int(111111111, 999999999);
                }

                $lastrandom = mb_substr($lastrandom, 0 - $lastdigits);
        ...
        case 'sql':
            ...
            if (preg_match('#Mozilla/(1|2|3|4)#', $HTTP_SERVER_VARS['HTTP_USER_AGENT']) && false === strpos($HTTP_SERVER_VARS['HTTP_USER_AGENT'], "compatible")) {
