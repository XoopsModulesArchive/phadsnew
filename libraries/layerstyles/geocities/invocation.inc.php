...
function phpAds_placeLayerSettings()
{
    ...
    echo "<input class='flat' type='text' name='collapsetime' size='' value='" . ($collapsetime ?? '-') . "' style='width:60px;' tabindex='" . ($tabindex++) . "'> " . $GLOBALS['strAbbrSeconds'] . '</td></tr>';
    ...
    echo "<input class='flat' type='text' name='padding' size='' value='" . ($padding ?? '0') . "' style='width:60px;' tabindex='" . ($tabindex++) . "'> " . $GLOBALS['strAbbrPixels'] . '</td></tr>';
...
function phpAds_generateLayerCode($parameters)
{
    ...
    if (isset($closetext)) {
        $parameters[] = 'closetext=' . urlencode($closetext);
    }
