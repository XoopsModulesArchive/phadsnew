...
function phpAds_placeLayerSettings()
{
    ...
    echo "<option value='left'" . ('left' == $align ? ' selected' : '') . '>' . $GLOBALS['strLeft'] . '</option>';

    echo "<option value='center'" . ('center' == $align ? ' selected' : '') . '>' . $GLOBALS['strCenter'] . '</option>';

    echo "<option value='right'" . ('right' == $align ? ' selected' : '') . '>' . $GLOBALS['strRight'] . '</option>';
    ...
    echo "<option value='top'" . ('top' == $valign ? ' selected' : '') . '>' . $GLOBALS['strTop'] . '</option>';

    echo "<option value='middle'" . ('middle' == $valign ? ' selected' : '') . '>' . $GLOBALS['strMiddle'] . '</option>';

    echo "<option value='bottom'" . ('bottom' == $valign ? ' selected' : '') . '>' . $GLOBALS['strBottom'] . '</option>';
    ...
    echo "<option value='t'" . ('t' == $closebutton ? ' selected' : '') . '>' . $GLOBALS['strYes'] . '</option>';

    echo "<option value='f'" . ('f' == $closebutton ? ' selected' : '') . '>' . $GLOBALS['strNo'] . '</option>';
    ...
    echo "<input class='flat' type='text' name='closetime' size='' value='" . ($closetime ?? '-') . "' style='width:60px;' tabindex='" . ($tabindex++) . "'> " . $GLOBALS['strAbbrSeconds'] . '</td></tr>';
    ...
    echo "<input type='text' class='flat' name='backcolor' size='10' maxlength='7' tabindex='" . ($tabindex++) . "' value='" . $backcolor . "' onFocus='current_cp = this; current_cp_oldval = this.value; current_box = backcolor_box' onChange='c_update()'" . ('t' == $nobg ? ' disabled' : '') . '>';

    echo "</td><td align='right' width='218'>";

    echo "<div id='backDiv'"
         . ('t' == $nobg ? " style='display: none'" : '')
         . " onMouseOver='current_cp = backcolor; current_box = backcolor_box' onMouseOut='current_cp = null'><img src='images/colorpicker.png' width='193' height='18' align='absmiddle' usemap='#colorpicker' border='0'><img src='images/spacer.gif' width='22' height='1'></div>";
    ...
    echo "<input type='text' class='flat' name='bordercolor' size='10' maxlength='7' tabindex='"
         . ($tabindex++)
         . "' value='"
         . $bordercolor
         . "' onFocus='current_cp = this; current_cp_oldval = this.value; current_box = bordercolor_box' onChange='c_update()'"
         . ('t' == $noborder ? ' disabled' : '')
         . '>';

    echo "</td><td align='right' width='218'>";

    echo "<div id='borderDiv'"
         . ('t' == $noborder ? " style='display: none'" : '')
         . " onMouseOver='current_cp = bordercolor; current_box = bordercolor_box' onMouseOut='current_cp = null'><img src='images/colorpicker.png' width='193' height='18' align='absmiddle' usemap='#colorpicker' border='0'><img src='images/spacer.gif' width='22' height='1'></div>";
    ...
    echo "<input type='checkbox' name='nobg' value='t' tabindex='" . ($tabindex++) . "' onClick='this.form.backcolor.disabled=this.checked;backDiv.style.display=this.checked?\"none\":\"\"'" . ('t' == $nobg ? ' checked' : '') . '>&nbsp;';
    ...
    echo "<input type='checkbox' name='noborder' value='t' tabindex='" . ($tabindex++) . "' onClick='this.form.bordercolor.disabled=this.checked;borderDiv.style.display=this.checked?\"none\":\"\"'" . ('t' == $noborder ? ' checked' : '') . '>&nbsp;';
...
function phpAds_generateLayerCode($parameters)
{
    ...
    if (isset($backcolor)) {
        $parameters[] = 'backcolor=' . mb_substr($backcolor, 1);
    }

    if (isset($bordercolor)) {
        $parameters[] = 'bordercolor=' . mb_substr($bordercolor, 1);
    }
...
function toHex($d)
{
    return mb_strtoupper(sprintf('%02x', $d));
}
