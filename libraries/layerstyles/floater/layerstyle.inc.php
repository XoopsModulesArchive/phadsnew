...
function phpAds_getLayerLimitations()
{
    ...
    $richmedia = 'IE' == $agent['agent'] && $agent['version'] > 5.0 && 'Win' == $agent['platform'] ? true : false;
...
function phpAds_putLayerJS($output, $uniqid)
{
    ...
    if (mb_strstr($HTTP_SERVER_VARS['HTTP_USER_AGENT'], 'Opera 6')) {
        ?>
        mr = mr - w; ml = ml + w;
    ...
    if ('t' == $ltr) {
        ...
        if (<?php echo 'n' == $loop ? 'true' : 'phpAds_adlayers_counter_' . $uniqid . ' < ' . $loop; ?>)
...
function phpAds_getLayerHTML($output, $uniqid)
{
    ...
    return '<div id="phpads_'
           . $uniqid
           . '" style="position:absolute; width:'
           . $output['width']
           . 'px; height:'
           . $output['height']
           . 'px; z-index:99; left: 0px; top: '
           . $shiftv
           . 'px; visibility: hidden; overflow: hidden'
           . ('t' == $transparent ? '' : '; background-color: "'
                                         . $backcolor
