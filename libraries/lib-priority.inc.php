...
function phpAds_PriorityPredictProfile($campaigns, $banners)
{
    ...
    if ($days_running >= 8) {
        ...
        if ($impressions_last_week > 0) {
            ...
            for ($i = 0, $iMax = count($profile); $i < $iMax; $i++) {
    ...
    if ($total_profile > 0) {
        if (phpAds_CurrentHour > 0) {
            ...
            for ($i = 0; $i < 24; $i++) {
                $predicted_today += $profile[$i] ?? 0;
            }

            $predicted_up_till_now = 0;

            for ($i = 0; $i < phpAds_CurrentHour; $i++) {
                $predicted_up_till_now += $profile[$i] ?? 0;
            }
                ...
                while (!$profile_correction_done) {
                    for ($i = phpAds_CurrentHour; $i > 0; $i--) {
                        ...
                        if ($deviance > 2.25) {
                            ...
                            $deviance = (phpAds_PriorityGetDeviance($k, $profile, $real_profile) + phpAds_PriorityGetDeviance(phpAds_CurrentHour == $k ? $k : $k + 1, $profile, $real_profile)) / 2;

                            for ($j = 0; $j < $k; $j++) {
                                $profile[$j] = ($profile[$j] ?: 1) * $deviance;
    ...
    return [$profile, $profile_correction_executed];
}
...
function phpAds_PriorityStore($banners, $campaigns = '')
{
    ...
    for (reset($banners); $b = key($banners); next($banners)) {
        $query = '
			UPDATE ' . $phpAds_config['tbl_banners'] . '
			SET priority = ' . ($banners[$b]['priority'] ?? 0) . '
...
function phpAds_PriorityCalculate()
{
        ...
        if ($campaigns[$c]['target'] > 0) {
            for (reset($banners); $b = key($banners); next($banners)) {
                if ($banners[$b]['parent'] == $c) {
                    $targeted_hits += $banners[$b]['hits'] ?? 0;
                }
            }
    ...
    if ($total_requested) {
        ...
        [$profile, $profile_correction_executed] = phpAds_PriorityPredictProfile($campaigns, $banners);
        ...
        for (reset($campaigns); $c = key($campaigns); next($campaigns)) {
            if ($campaigns[$c]['target'] > 0) {
                ...
                for ($p = 0; $p < $maxperiod; $p++) {
                    $total_profile += $profile[$p] ?? 0;
                }

                $profile_uptil_now = 0;

                for ($p = 0; $p < $period; $p++) {
                    $profile_uptil_now += $profile[$p] ?? 0;
                }
...
function phpAds_PriorityGetDeviance($hour, $profile, $real_profile)
{
    ...
    for ($i = 0; $i < $hour; $i++) {
        $predicted += $profile[$i] ?? 0;

        $real += $real_profile[$i] ?? 0;
    }
...
function phpAds_PriorityPrintProfile($profile)
{
    ...
    for ($i = 0; $i < 24; $i++) {
        ...
        $debuglog .= sprintf('%7d', $profile[$i] ?? '0') . '  ';
    }
...
function phpAds_PriorityTotalWeight($campaigns, $banners)
{
    ...
    if (0 == $tcw) {
        return 0;
    }  // All campaigns are disabled or high priority

    // Get total banner weight for each campaign

    for (reset($banners); $b = key($banners); next($banners)) {
        if (isset($campaigns[$banners[$b]['parent']]) && 't' == $campaigns[$banners[$b]['parent']]['active']) {
    ...
    for (reset($banners); $b = key($banners); next($banners)) {
        if (isset($campaigns[$banners[$b]['parent']]) && 't' == $campaigns[$banners[$b]['parent']]['active'] && $campaigns[$banners[$b]['parent']]['weight'] > 0) {
    ...
    if (0 == $min) {
        return 0;
    } // No active banners in low priority campaigns
