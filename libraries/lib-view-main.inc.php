...
function view_raw($what, $clientid = 0, $target = '', $source = '', $withtext = 0, $context = 0, $richmedia = true)
{
    ...
    while (($first || '' != $what) && false === $found) {
        $first = false;

        if ('zone:' == mb_substr($what, 0, 5)) {
    ...
    if ($found) {
        ...
        if (isset($HTTP_SERVER_VARS['HTTP_HOST'])) {
            if (preg_match('#//[^/]+:[0-9]+/#', $phpAds_config['url_prefix'])) {
                ...
                if (mb_strpos($HTTP_SERVER_VARS['HTTP_HOST'], ':') > 0) {
                    [$real_host] = explode(':', $HTTP_SERVER_VARS['HTTP_HOST']);
                } else {
        ...
        if ('html' == $row['storagetype']
            || 'url' == $row['storagetype']
            || 'network' == $row['storagetype']) {
            // Replace timestamp
        ...
        if (isset($HTTP_SERVER_VARS['HTTP_USER_AGENT']) && preg_match('#Mozilla/(1|2|3|4)#', $HTTP_SERVER_VARS['HTTP_USER_AGENT']) && false === strpos($HTTP_SERVER_VARS['HTTP_USER_AGENT'], "compatible")) {
            $outputbuffer .= '<layer id="beacon_' . $row['bannerid'] . '" width="0" height="0" border="0" visibility="hide">';

            $outputbuffer .= '<img src=\''
...
{
    $output = view_raw($what, $clientid, (string)$target, (string)$source, $withtext, $context, $richmedia);
