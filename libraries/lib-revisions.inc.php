...
function phpAds_revisionCheck()
{
    ...
    if ($revfile = fopen(phpAds_path . '/libraries/defaults/revisions.txt', 'rb')) {
        ...
        if ($version == $phpAds_version) {
            while (!feof($revfile)) {
                ...
                if ('' != trim($line)) {
                    [$filename, $rev, $md5] = explode("\t", $line);

                    // Get actual info from file

                    if (@file_exists(phpAds_path . $filename)) {
                        [$current_rev, $current_md5] = phpAds_revisionGet(phpAds_path . $filename);
                        ...
                        if (trim($current_md5) != trim($md5)) {
                            ...
                            switch ($result) {
                                case  1:
                                    break;  // File has been patched with a newer revision... that's allowed!
                                case  0:
                                    if (isset($GLOBALS['strRevCorrupt'])) {
    ...
    if (!file_exists(phpAds_path . '/language/english/default.lang.php')
        || !file_exists(phpAds_path . '/language/english/index.lang.php') || !file_exists(phpAds_path . '/language/english/invocation.lang.php') || !file_exists(phpAds_path . '/language/english/maintenance.lang.php') || !file_exists(phpAds_path . '/language/english/report.lang.php') || !file_exists(phpAds_path . '/language/english/settings-help.lang.php') || !file_exists(phpAds_path . '/language/english/settings.lang.php')
    ...
    if (count($message)) {
        ...
        $needed = [
                '/admin/upgrade.php',
                '/admin/install.php',
                '/libraries/lib-revisions.inc.php',
                '/libraries/lib-io.inc.php',
                '/libraries/lib-dbconfig.inc.php',
                '/libraries/lib-db.inc.php',
                '/admin/lib-install-db.inc.php',
                '/admin/lib-permissions.inc.php',
                '/admin/lib-gui.inc.php',
...
function phpAds_revisionCreate()
{
    ...
    if ($revfile = fopen(phpAds_path . '/libraries/defaults/revisions.txt', 'wb')) {
        // Determine the version of phpAdsNew
...
function phpAds_revisionScan($revfile, $path)
{
    if ($dir = opendir($path)) {
        while (false !== ($file = readdir($dir))) {
            if (is_dir($path . $file)) {
                if ('.' != $file && '..' != $file && 'CVS' != $file && 'language' != $file) {
                    phpAds_revisionScan($revfile, $path . $file . '/');
                }
            } else {
                if ('config.inc.php' != $file && '.' != $file[0]) {
