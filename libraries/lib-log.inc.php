...
function phpAds_logExpire($clientid, $type = 0)
{
    ...
    $campaignresult = phpAds_dbQuery(
        'SELECT *, UNIX_TIMESTAMP(expire) AS expire_st, UNIX_TIMESTAMP(activate) AS activate_st FROM ' . $phpAds_config['tbl_clients'] . " WHERE clientid = '" . $clientid . "'"
    );

    if ($campaign = phpAds_dbFetchArray($campaignresult)) {
        // Decrement views

        if (phpAds_Views == $type && $campaign['views'] > 0) {
        ...
        if (phpAds_Clicks == $type && $campaign['clicks'] > 0) {
        ...
        if ((0 == $campaign['clicks'])
            || (0 == $campaign['views']) || (time() < $campaign['activate_st'])
            || (time() > $campaign['expire_st'] && 0 != $campaign['expire_st'])) {
            $active = 'f';
        }
    ...
    if ($host = phpads_logCheckHost()) {
        ...
        if ($phpAds_config['compact_stats']) {
            $result = phpAds_dbQuery(
                'UPDATE ' . ($phpAds_config['insert_delayed'] ? 'LOW_PRIORITY' : '') . ' ' . $phpAds_config['tbl_adstats'] . " SET views = views + 1 WHERE day = NOW() 
            ...
            if (0 == phpAds_dbAffectedRows()) {
                $result = phpAds_dbQuery(
                    'INSERT ' . ($phpAds_config['insert_delayed'] ? 'DELAYED' : '') . ' INTO ' . $phpAds_config['tbl_adstats'] . " SET clicks = 0, views = 1, day = NOW(),
            ...
            $result = phpAds_dbQuery(
                'INSERT ' . ($phpAds_config['insert_delayed'] ? 'DELAYED' : '') . ' INTO ' . $phpAds_config['tbl_adviews'] . " SET bannerid = '" . $bannerid . "', 
...
function phpAds_logClick($bannerid, $clientid, $zoneid, $source)
{
    ...
    if ($host = phpads_logCheckHost()) {
        ...
        if ($phpAds_config['compact_stats']) {
            $result = phpAds_dbQuery(
                'UPDATE ' . ($phpAds_config['insert_delayed'] ? 'LOW_PRIORITY' : '') . ' ' . $phpAds_config['tbl_adstats'] . " SET clicks = clicks + 1 WHERE day = NOW() AND
            ...
            if (0 == phpAds_dbAffectedRows()) {
                $result = phpAds_dbQuery(
                    'INSERT ' . ($phpAds_config['insert_delayed'] ? 'DELAYED' : '') . ' INTO ' . $phpAds_config['tbl_adstats'] . " SET clicks = 1, views = 0, day = NOW(),
					hour = HOUR(NOW()), bannerid = '" . $bannerid . "', zoneid = '" . $zoneid . "',
					source = '" . $log_source . "' "
                );
            }
            ...
            $result = phpAds_dbQuery(
                'INSERT ' . ($phpAds_config['insert_delayed'] ? 'DELAYED' : '') . ' INTO ' . $phpAds_config['tbl_adclicks'] . " SET bannerid = '" . $bannerid . "', zoneid = '" . $zoneid . "',
