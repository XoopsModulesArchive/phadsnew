...
function phpAds_aclCheckWeekday($data, $ad)
{
    ...
    $expression = ('*' == $data || $data == $day || in_array($day, explode(',', $data), true));
...
function phpAds_aclCheckClientip($data, $ad)
{
    ...
    if (!mb_strpos($data, '/')) {
        ...
        for ($i = 0, $iMax = count($net); $i < $iMax; $i++) {
            if ('*' == $net[$i]) {
                $net[$i] = 0;
...
function phpAds_aclCheckDomain($data, $ad)
{
    ...
    $domain = mb_substr($host, -(mb_strlen($data)));

    $expression = ('*' == $data || mb_strtolower($domain) == mb_strtolower($data));
...
function phpAds_aclCheckSource($data, $ad, $source)
{
    ...
    $expression = ('*' == $data || mb_strtolower($source) == mb_strtolower($data)
...
function phpAds_aclCheckTime($data, $ad)
{
    ...
    $expression = ('*' == $data || $data == $time || in_array($time, explode(',', $data), true));
...
function phpAds_aclCheckCountry($data, $ad)
{
    ...
    if ($phpAds_geo && $phpAds_geo['country']) {
        ...
        $expression = ($data == $phpAds_geo['country'] || in_array($phpAds_geo['country'], explode(',', $data), true));
...
function phpAds_aclCheckContinent($data, $ad)
{
    ...
    if ($phpAds_geo && $phpAds_geo['continent']) {
        ...
        $expression = ($data == $phpAds_geo['continent'] || in_array($phpAds_geo['continent'], explode(',', $data), true));
...
function phpAds_aclCheckRegion($data, $ad)
{
    ...
    if ($phpAds_geo && 'US' == $phpAds_geo['country']) {
        ...
        $expression = ($data == $phpAds_geo['region'] || in_array($phpAds_geo['region'], explode(',', $data), true));
...
function phpAds_aclCheckReferer($data, $ad)
{
    ...
    $referer = isset($HTTP_SERVER_VARS['HTTP_REFERER']) ? mb_strtolower($HTTP_SERVER_VARS['HTTP_REFERER']) : '';

    $expression = mb_strpos($referer, mb_strtolower($data));
