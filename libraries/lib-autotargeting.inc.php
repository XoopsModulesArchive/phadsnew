...
{
    $days = round(($expire - phpAds_LastMidnight) / (float)(60 * 60 * 24));

    $daily_target = ceil($views / $days);

    if (1 == $days) { // Quite easy... use remaining views as last day target
        return [$views, '(last day, using remaining views)'];
    }

    if ($days <= 0) { // Campaign expired
        return [0, '- campaign is expired, is there a bug?'];
    }

    if (-1 == $factor) { // Use old style average targeting
        return [$daily_target, '(average targeting)'];
    }
    ...
    if ($days < 7) {
        ...
        if ($expire_dow < phpAds_DowToday) {
            // Expire date is after the week end

            $predicted_views = phpAds_AutoTargetingProfileSum($profile, 7, phpAds_DowToday) + phpAds_AutoTargetingProfileSum($profile, $expire_dow);
        } else {
...
function phpAds_AutoTargetingPrepareProfile($weeks = 2)
{
    ...
    if ($days_running < $weeks * 7) {
        if ($days_running < 7) { // Not enough stats
            return $profile;
    ...
    while (false !== ($row = phpAds_dbFetchArray($res_views))) {
        // PostgreSQL dow starts with 1 instead of 0

        if ('PostgreSQL' == $phpAds_dbmsname) {
            $row['dow']--;
        }
...
function phpAds_AutoTargetingCaclulateFactor($profile, &$debuglog)
{
    ...
    while (false !== ($row = phpAds_dbFetchArray($res))) {
        ...
        if ($row['expire']) {
            ...
            $days = ($row['expire'] - phpAds_LastMidnight) / (float)(60 * 60 * 24);
