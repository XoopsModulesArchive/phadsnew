...
function phpAds_SendMaintenanceReport($clientid, $first_unixtimestamp, $last_unixtimestamp, $update = true)
{
    ...
    if (phpAds_dbNumRows($res_client) > 0) {
		...
		'
        ) || die($strLogErrorClients);

        while (false !== ($campaign = phpAds_dbFetchArray($res_campaigns))) {
            $current_log = '';
            ...
            while (false !== ($row_banners = phpAds_dbFetchArray($res_banners))) {
                ...
                if ($adviews > 0) {
                    ...
                    if (phpAds_dbNumRows($res_adviews)) {
                        ...
                        while (false !== ($row_adviews = phpAds_dbFetchArray($res_adviews))) {
                ...
                if ($adclicks > 0) {
                    ...
                    if ($phpAds_config['compact_stats']) {
                        $res_adclicks = phpAds_dbQuery(
							...
							ORDER BY
								day DESC
							"
                        ) || die("$strLogErrorClicks " . phpAds_dbError());
                    } else {
                        $res_adclicks = phpAds_dbQuery(
                            "
							SELECT
                    ...
                    if (phpAds_dbNumRows($res_adviews)) {
                        ...
                        while (false !== ($row_adclicks = phpAds_dbFetchArray($res_adclicks))) {
                ...
                if (0 == $adclicks && 0 == $adviews) {
                    $current_log .= '      ' . $strNoStatsForCampaign . "\n";
                }

                $current_log .= "\n\n";

                if ('t' == $campaign['active'] || true === $active_banner_stats) {
                    $active_banners = true;
                }
            }

            if (true === $active_banners) {
        ...
        if ('' != $client['email'] && true === $active_campaigns) {
