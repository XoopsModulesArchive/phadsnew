...
if (!isset($bannerid)) {
    ...
    if (isset($HTTP_COOKIE_VARS['phpAds_banner'][$n])
        && 'DEFAULT' != $HTTP_COOKIE_VARS['phpAds_banner'][$n]) {
...
if (phpAds_dbConnect()) {
    if ('DEFAULT' != $bannerid) {
        ...
        if (!isset($log) || 'no' != $log) {
            if (0 == $phpAds_config['block_adclicks']
                ...
                if ($phpAds_config['block_adclicks'] > 0) {
                    phpAds_setCookie(
                        'phpAds_blockClick[' . $bannerid . ']',
                        time() + $phpAds_config['block_adclicks'],
        ...
        if (isset($_GET)) {
            for (reset($_GET); $key = key($_GET); next($_GET)) {
                if ('bannerid' != $key
                    && 'zoneid' != $key && 'source' != $key && 'dest' != $key && 'ismap' != $key && 'log' != $key && 'n' != $key
                    && 'cb' != $key) {
                    $vars[] = $key . '=' . $_GET[$key];
                }
            }
